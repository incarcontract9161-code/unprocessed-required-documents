...
# ==========================================
# 4. 공문 생성 함수 (매뉴얼 첨부 포함 + 차트 삽입 + 가독성 개선)
# ==========================================
...
def generate_agent_report_pdf(title, dept, sender_name, date_str, recipient, table_data, special_notes, actual_rate, target_rate):
    ...
    # ✅ 발신자 수기 입력 가능하게 (sender_name 파라미터 사용)
    ...
    # ✅ 실적 vs 목표 비교 차트 생성 및 삽입 (가로 막대 그래프)
    try:
        fig = go.Figure()
        ...
        fig.update_layout(barmode='group', height=150, margin=dict(l=80, r=20, t=20, b=30), xaxis=dict(range=[0, max(actual_rate, target_rate)*1.3]))
        img_bytes = fig_to_png_bytes(fig, width=500, height=150, scale=2)
        if img_bytes:
            img = RLImage(io.BytesIO(img_bytes), width=140*mm, height=40*mm)
...
def dashboard_page():
    ...
    # ==========================================
    # 탭 3: 목표관리 & 공문출력 (✅ 수정: 지원센터 발신자 입력, PDF 개선)
    # ==========================================
    with tab_target:
                    ...
                    if st.button("🖨️ 공문 생성 (PDF)", use_container_width=True, type="primary"):
                        with st.spinner("📄 공문 생성 중..."):
                            try:
                                ...
                                # ✅ PDF 테이블 데이터: 실적을 현황으로 변경, 특이사항 행 제거 로직은 함수 내에서 처리
                                table_data = [
                                    ['지표', '목표', '현황', '차이'],
...
def main():
    if not st.session_state.get("logged_in"): login_page()
        ...
        with st.sidebar:
            ...
            st.divider()
            st.caption("v15.3 | 매뉴얼첨부완비 | 공문개선(현황/가로그래프/특이사항/담당자인제거) | 발신자입력")
        dashboard_page()

...
def load_data():
    ...
    converters={7: str} )       
        ...
        # FA고지 대상 여부 판정 (해당없음/빈값/공백 제외 - 완전판매와 동일 방식)
        def is_fa_target(val):
            if pd.isna(val):
                return False
            val = str(val).strip()
            return val in ["스캔", "M스캔", "보험사스캔", "미스캔"]

        # 비교설명 대상 여부 판정 (해당없음/빈값/공백 제외 - 완전판매와 동일 방식)
        def is_bi_target(val):
            if pd.isna(val):
                return False
            val = str(val).strip()
            return val in ["스캔", "M스캔", "보험사스캔", "미스캔"]
        ...
        df["FA_target"] = df["FA고지_c"].apply(is_fa_target).astype(int)
        df["비교_target"] = df["비교설명_c"].apply(is_bi_target).astype(int)
...
def _style_pdf_chart(fig, font_name, height=360, bottom_margin=110):
    fig.update_layout(
        ...
        font=dict(family=font_name, size=10, color="#1F1F1F"),
        plot_bgcolor="white",
        paper_bgcolor="white",
        height=height,
        margin=dict(l=30, r=30, t=60, b=bottom_margin),
...
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="group", dash_top_n=15, map_type="treemap", view_mode="조직별 보기", show_breakdown=False):
    fn = register_korean_font()
    st_ = _pdf_styles(fn)
    buf = io.BytesIO()
    ...
    chart_org_col = "_chart_org_label"
    agg[chart_org_col] = agg[org_label].astype(str).apply(lambda x: _wrap_chart_label(x, 7))
    ...
    if not agg.empty:
        ...
        try:
            ...
            if len(dash_doc_types) == 1:
                ...
                fig_dash.update_coloraxes(showscale=False)
                ...
                chart_df = agg[[chart_org_col] + metric_cols].copy()
            ...
            _style_pdf_chart(fig_dash, fn, height=380, bottom_margin=115)
            fig_dash.update_layout(uniformtext_minsize=8, uniformtext_mode="hide", bargap=0.22)
            _append_pdf_figure(elements, fig_dash, st_, max_width=1000, height=380)
        ...
        try:
            ...
            trend_fig.update_layout(title=f"미처리율 / 스캔율 TOP {dash_top_n}")
            _style_pdf_chart(trend_fig, fn, height=340, bottom_margin=115)
            trend_fig.update_yaxes(ticksuffix="%")
            _append_pdf_figure(elements, trend_fig, st_, max_width=1000, height=340)
        except Exception as err:
    ...
    # ══════════════════════════════════════════════════════════════════
    elements.append(PageBreak())
    elements.append(Paragraph(f"대시보드 차트 — {period_str}", st_["title"]))
    elements.append(Paragraph(f"보기 모드: {view_mode} | 선택 서류: {', '.join(dash_doc_types)} | TOP {dash_top_n}", st_["date"]))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor(HDR_CLR)))
    elements.append(Spacer(1, 8))

    # ── ① 조직별 TOP N 차트 ────────────────────────────────────────────
    try:
        elements.append(Paragraph(f"① {agg_group} 기준 미처리건수 TOP {dash_top_n}", st_["section"]))
        metric_map_fp = {
            total_label: "total_miss",
            fa_label: "fa_miss_sum",
            bi_label: "bi_miss_sum",
        ...
        if len(dash_doc_types) == 1:
            mk = metric_map_fp.get(dash_doc_types[0], "total_miss")
            cs = "Reds" if mk == "total_miss" else "Blues"
            fig_top = px.bar(
                agg, x=chart_org_col, y=mk,
                title=f"{agg_group} 기준 {dash_doc_types[0]} TOP {dash_top_n}",
                text=mk, color=mk, color_continuous_scale=cs,
            )
            fig_top.update_coloraxes(showscale=False)
            fig_top.update_traces(texttemplate="%{text:,.0f}", textposition="outside",
                                  cliponaxis=False, marker_line_color="#FFFFFF", marker_line_width=0.7)
        else:
            metric_cols_fp = [metric_map_fp.get(d, "total_miss") for d in dash_doc_types]
            chart_df_fp = agg[[chart_org_col] + metric_cols_fp].copy()
            chart_df_fp.columns = [org_label] + dash_doc_types
            chart_df_fp = chart_df_fp.melt(org_label, var_name="서류", value_name="건수")
            bm = "group" if dash_chart_mode in ("group", "그룹형") else "stack"
            fig_top = px.bar(
                chart_df_fp, x=org_label, y="건수", color="서류", barmode=bm,
                title=f"{agg_group} 기준 서류별 미스캔 TOP {dash_top_n}",
                text="건수",
                color_discrete_map={total_label:"#C0392B", fa_label:"#2E86C1", bi_label:"#8E44AD", cs_label:"#27AE60"},
            )
            fig_top.update_traces(texttemplate="%{text:,.0f}", textposition="outside",
                                  cliponaxis=False, marker_line_color="#FFFFFF", marker_line_width=0.7)
        _style_pdf_chart(fig_top, fn, height=340, bottom_margin=110)
        fig_top.update_layout(uniformtext_minsize=8, uniformtext_mode="hide", bargap=0.22)
        _append_pdf_figure(elements, fig_top, st_, max_width=1000, height=340)
    except Exception as e:
        elements.append(Paragraph(f"① 조직별 차트 생성 오류: {e}", st_["notice"]))
    elements.append(Spacer(1, 6))

    # ── ② 월별 미처리건수(Bar) + 스캔율(Line) 이중축 복합 차트 ─────────
    try:
        elements.append(Paragraph("② 전사 기준 월별 미처리건수 & 스캔율 추이", st_["section"]))
        mon_grp = df_sel.groupby(month_col).agg(
            대상건=(target_col, "sum"),
            스캔건=(scan_col, "sum"),
            미스캔=(miss_col, "sum"),
            FA_miss=("FA_miss", "sum"),
            비교_miss=(bi_miss_col, "sum"),
            완판_miss=(cs_miss_col, "sum"),
        ).reset_index().sort_values(month_col)
        mon_grp["스캔율"] = ((mon_grp["스캔건"] / mon_grp["대상건"].replace(0, pd.NA)) * 100).round(1).fillna(0.0)
        mon_grp["미처리율"] = ((mon_grp["미스캔"] / mon_grp["대상건"].replace(0, pd.NA)) * 100).round(1).fillna(0.0)

        from plotly.subplots import make_subplots as _make_subplots
        fig_mon = _make_subplots(specs=[[{"secondary_y": True}]])

        if show_breakdown:
            for col_key, lbl, clr in [("FA_miss","FA고지","#2E86C1"),("비교_miss","비교설명","#8E44AD"),("완판_miss","완전판매","#27AE60")]:
                fig_mon.add_trace(go.Bar(
                    x=mon_grp[month_col], y=mon_grp[col_key], name=lbl,
                    text=mon_grp[col_key].apply(lambda v: f"{int(v):,}"),
                    textposition="inside", textfont=dict(size=8, color="white"),
                    hovertemplate=f"%{{x}}<br>{lbl}: %{{y:,.0f}}건<extra></extra>",
                    marker_color=clr,
                ), secondary_y=False)
            bar_mode_mon = "stack"
        else:
            fig_mon.add_trace(go.Bar(
                x=mon_grp[month_col], y=mon_grp["미스캔"], name="미처리건수",
                text=mon_grp["미스캔"].apply(lambda v: f"{int(v):,}"),
                textposition="outside", textfont=dict(size=9),
                hovertemplate="%{x}<br>미처리건수: %{y:,.0f}건<extra></extra>",
                marker_color="#E74C3C", opacity=0.85,
            ), secondary_y=False)
            bar_mode_mon = "group"

        fig_mon.add_trace(go.Scatter(
            x=mon_grp[month_col], y=mon_grp["스캔율"], name="스캔율(%)",
            mode="lines+markers+text",
            text=[f"{v:.1f}%" for v in mon_grp["스캔율"]],
            textposition="top center", textfont=dict(size=9, color="#1A5276"),
            hovertemplate="%{x}<br>스캔율: %{y:.1f}%<extra></extra>",
            line=dict(color="#1A5276", width=3, shape="spline"), marker=dict(size=7, color="#1A5276"),
        ), secondary_y=True)

        fig_mon.add_trace(go.Scatter(
            x=mon_grp[month_col], y=mon_grp["미처리율"], name="미처리율(%)",
            mode="lines+markers+text",
            text=[f"{v:.1f}%" for v in mon_grp["미처리율"]],
            textposition="bottom center", textfont=dict(size=8, color="#922B21"),
            hovertemplate="%{x}<br>미처리율: %{y:.1f}%<extra></extra>",
            line=dict(color="#922B21", width=2, dash="dot", shape="spline"), marker=dict(size=6, color="#922B21"),
        ), secondary_y=True)

        fig_mon.update_layout(
            title=f"전사 기준 월별 미처리건수 & 스캔율 ({period_str})",
            barmode=bar_mode_mon, height=360, hovermode="x unified",
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
            plot_bgcolor="white", paper_bgcolor="white",
            font=dict(family=fn, size=10),
            margin=dict(l=30, r=30, t=70, b=50),
        )
        fig_mon.update_yaxes(title_text="미처리건수(건)", secondary_y=False,
                              showgrid=True, gridcolor="#F0F0F0", tickformat=",")
        fig_mon.update_yaxes(title_text="처리율(%)", secondary_y=True,
                              range=[0, 110], ticksuffix="%", showgrid=False)
        fig_mon.update_xaxes(type="category", tickangle=-30)   # 2026-01 형식 고정
        _append_pdf_figure(elements, fig_mon, st_, max_width=1000, height=360)
    except Exception as e:
        elements.append(Paragraph(f"② 월별 복합 차트 생성 오류: {e}", st_["notice"]))
    elements.append(Spacer(1, 6))

    # ── ③ 서류별(FA/비교/완판) 월간 추이 꺾은선 ───────────────────────
    try:
        elements.append(Paragraph("③ 전사 기준 서류별 월간 미처리건수 추이 (FA고지 / 비교설명 / 완전판매)", st_["section"]))
        fig_doc = go.Figure()
        for col_key, lbl, clr in [("FA_miss","FA고지","#2E86C1"),("비교_miss","비교설명","#8E44AD"),("완판_miss","완전판매","#27AE60")]:
            fig_doc.add_trace(go.Scatter(
                ...
                hovertemplate=f"%{{x}}<br>{lbl}: %{{y:,.0f}}건<extra></extra>",
                line=dict(color=clr, width=2, shape="spline"), marker=dict(size=6, color=clr),
            ))
        fig_doc.update_layout(
            title=f"전사 기준 서류별 월간 미처리건수 추이 ({period_str})",
            ...
            xaxis=dict(type="category", tickangle=-30),   # 2026-01 형식 고정
        )
        _append_pdf_figure(elements, fig_doc, st_, max_width=1000, height=320)
    except Exception as e:
        elements.append(Paragraph(f"③ 서류별 추이 차트 생성 오류: {e}", st_["notice"]))
...
# ==========================================
# 11-A. [추가] 월별 추이 차트 생성 헬퍼
# ==========================================
@st.cache_data(ttl=300)
def build_monthly_trend_data(df, months, agg_group):
    """
    선택한 조직 계층 기준으로 월별 미처리건수 + 스캔율을 집계한 DataFrame 반환.
    X축: 월_피리어드 (정렬됨)
    컬럼: 월_피리어드, 대상건, 스캔건, 미스캔, 스캔율
    """
    src = df[df["월_피리어드"].isin(months)].copy() if months else df.copy()
    if src.empty:
        return pd.DataFrame()
    grp = src.groupby("월_피리어드").agg(
        대상건=("대상건", "sum"),
        스캔건=("스캔건", "sum"),
        미스캔=("미스캔", "sum"),
        FA_miss=("FA_miss", "sum"),
    ...
    grp["스캔율"] = ((grp["스캔건"] / grp["대상건"].replace(0, pd.NA)) * 100).round(1).fillna(0.0)
    grp["미처리율"] = ((grp["미스캔"] / grp["대상건"].replace(0, pd.NA)) * 100).round(1).fillna(0.0)
    grp = grp.sort_values("월_피리어드").reset_index(drop=True)
    return grp


def render_monthly_trend_chart(df_trend, org_label, show_doc_breakdown=False):
    """
    월별 미처리건수(Bar) + 스캔율(Line) 이중축 차트.
    show_doc_breakdown=True 이면 FA/비교/완판 별 스택바 추가.
    """
    if df_trend.empty:
        st.info("월별 집계 데이터가 없습니다.")
        return

    fig = make_subplots(specs=[[{"secondary_y": True}]])

    if show_doc_breakdown:
        # ── 서류별 스택 Bar ──────────────────────────────────────
        colors_doc = {"FA고지": "#2E86C1", "비교설명": "#8E44AD", "완전판매": "#27AE60"}
        for col_key, label in [("FA_miss", "FA고지"), ("비교_miss", "비교설명"), ("완판_miss", "완전판매")]:
            fig.add_trace(
                go.Bar(
                    x=df_trend["월_피리어드"],
                    y=df_trend[col_key],
                    name=label,
                    text=df_trend[col_key].apply(lambda v: f"{int(v):,}"),
                    textposition="inside",
                    textfont=dict(size=9, color="white"),
                    hovertemplate=f"%{{x}}<br>{label}: %{{y:,.0f}}건<extra></extra>",
                    marker_color=colors_doc[label],
                ),
                secondary_y=False,
            )
    else:
        ...
        fig.add_trace(
            go.Bar(
                ...
                y=df_trend["미스캔"],
                name="미처리건수",
                text=df_trend["미스캔"].apply(lambda v: f"{int(v):,}"),
                textposition="outside",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="스캔율(%)",
            mode="lines+markers+text",
            text=[f"{v:.1f}%" for v in df_trend["스캔율"]],
            textposition="top center",
            textfont=dict(size=10, color="#1A5276"),
            hovertemplate="%{x}<br>스캔율: %{y:.1f}%<extra></extra>",
            line=dict(color="#1A5276", width=3, shape="spline"),
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="미처리율(%)",
            mode="lines+markers+text",
            text=[f"{v:.1f}%" for v in df_trend["미처리율"]],
            textposition="bottom center",
            textfont=dict(size=9, color="#922B21"),
            hovertemplate="%{x}<br>미처리율: %{y:.1f}%<extra></extra>",
            line=dict(color="#922B21", width=2, dash="dot", shape="spline"),
            marker=dict(size=6, color="#922B21"),
        ),
        secondary_y=True,
    )

    barmode = "stack" if show_doc_breakdown else "group"
    fig.update_layout(
        title=dict(
            text=f"📅 월별 미처리건수 & 스캔율 추이 ({org_label} 기준)",
            font=dict(size=14),
        ),
        barmode=barmode,
        height=460,
        ...
        xaxis=dict(
            title="월",
            type="category",          # X축 2026-01 형식 고정
            tickangle=-30,
            showgrid=True,
            gridcolor="#F0F0F0",
        ),
    ...
    # ── 월별 수치 요약 테이블 ────────────────────────────────────
    with st.expander("📋 월별 수치 상세보기", expanded=False):
        display = df_trend[["월_피리어드", "대상건", "스캔건", "미스캔", "스캔율", "미처리율"]].copy()
        display.columns = ["월", "대상건", "스캔건", "미처리건수", "스캔율(%)", "미처리율(%)"]
        st.dataframe(
...
def dashboard_page():
    ...
    with tab_dash:
        ...
        if agg.empty:
            ...
            st.dataframe(
                agg[["순위", "조직", "대상건", "스캔건", "총_미스캔", "미처리율", "스캔율",
                      "FA고지_미스캔", "비교설명_미스캔", "완전판매_미스캔"]].style.format({
                    "순위": "{:,}", "대상건": "{:,}", "스캔건": "{:,}",
                    "총_미스캔": "{:,}", "미처리율": "{:.1f}%", "스캔율": "{:.1f}%",
                    "FA고지_미스캔": "{:,}", "비교설명_미스캔": "{:,}", "완전판매_미스캔": "{:,}",
                }),
                use_container_width=True,
                hide_index=True,
            )
            ...
            if view_mode == "조직별 보기":
                ...
                with c1:
                    ...
                    if doc_types:
                        ...
                        if len(doc_types) == 1 and doc_types[0] == "총 미스캔":
                            ...
                            fig = px.bar(
                                top, x="조직", y=col_map[doc_types[0]],
                                title=f"{doc_types[0]} 미스캔 TOP {top_n}",
                                text=col_map[doc_types[0]], color=col_map[doc_types[0]],
                            ...
                            fig = px.bar(
                                p, x="조직", y="건수", color="종류", text="건수",
                                barmode="group" if chart_mode == "그룹형" else "stack",
                            )
                            fig.update_traces(
                                texttemplate="%{text:,.0f}", textposition="outside",
                ...
                with c2:
                    ...
                    fig2.update_layout(
                        title=f"미처리 건수 추이 TOP {top_n}",
                        xaxis_tickangle=-45,
                        yaxis=dict(range=yr, tickformat=","),
                        height=420,
                    )
                    st.plotly_chart(fig2, use_container_width=True)

            # ══════════════════════════════════════════════════
            # [추가] 월별 보기 – 미처리건수(Bar) + 스캔율(Line) 이중축
            # ══════════════════════════════════════════════════
            else:
                # ── display_label 결정: 검색어 있으면 해당 조직, 없으면 "전사" ──
                display_label = "전사"
                if search_text:
                    display_label = search_text

                st.markdown(f"#### 📅 월별 미처리 추이 — **{display_label}** 기준 | 선택 월: {', '.join(sel_months)}")

                mo1, mo2 = st.columns([1, 1])
                with mo1:
                    show_breakdown = st.checkbox(
                        "📂 서류별(FA/비교/완판) 분리 표시",
                        value=False,
                        key="dash_month_breakdown",
                        help="체크 시 미처리건수를 FA고지 / 비교설명 / 완전판매별 스택바로 표시합니다.",
                    )
                with mo2:
                    month_filter = st.multiselect(
                        ...
                        help="선택한 월만 차트에 표시합니다.",
                    )

                # ── 전사 기준 월별 추이 ──────────────────────────────
                df_trend = build_monthly_trend_data(df_sel, month_filter or sel_months, agg_group)
                render_monthly_trend_chart(df_trend, display_label, show_doc_breakdown=show_breakdown)

                # ── 검색 조직 한정 추이 (검색어 있을 때만 추가 표시) ──
                if search_text and not agg.empty:
                    st.markdown("---")
                    st.markdown(f"#### 🔍 `{search_text}` 한정 월별 추이")
                    df_search_src = df_sel[df_sel[agg_group].astype(str).str.contains(search_text, case=False, na=False)]
                    if not df_search_src.empty:
                        df_trend_search = build_monthly_trend_data(df_search_src, month_filter or sel_months, agg_group)
                        render_monthly_trend_chart(df_trend_search, search_text, show_doc_breakdown=show_breakdown)
                    else:
        ...
        if report_df.empty:
            ...
            if st.button("\uc804\uccb4 \ud398\uc774\uc9c0 PDF", use_container_width=True, key="dl_fullpage_pdf"):
                with st.spinner("\uc804\uccb4 \ud398\uc774\uc9c0 PDF \uc0dd\uc131 \uc911..."):
                    ...
                    view_mode_state = st.session_state.get("dash_view_mode", "조직별 보기")
                    show_breakdown_state = st.session_state.get("dash_month_breakdown", False)
                    buf_full = report_fullpage_pdf(df, sel_months, agg_group_state, map_level_state, dash_doc_types, dash_chart_mode, dash_top_n, map_type_state, view_mode=view_mode_state, show_breakdown=show_breakdown_state)

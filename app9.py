...
# ✅ st.secrets 기반 비밀번호 관리 (클라우드 배포 시 필수)
try:
    CREDENTIALS = {
        "admin": {"password": st.secrets["admin_password"], "role": "admin"},
        "incar": {"password": st.secrets["user_password"], "role": "user"},
    }
except KeyError:
    # 로컬 테스트용 fallback
    CREDENTIALS = {
        "admin": {"password": "admin961", "role": "admin"},
        "incar": {"password": "incar123", "role": "user"},
    }
...
def register_korean_font():
    # Render(Ubuntu) & 로컬(Windows) 경로 동시 지원
    paths = [
    ...
    for path in paths:
        if os.path.exists(path):
            try:
                font_name = "CloudKorean"
                pdfmetrics.registerFont(TTFont(font_name, path))
                return font_name
            except Exception:
                continue
    st.warning("⚠️ 한글 폰트 경로 없음. 영문/기본 폰트로 대체됩니다.")
    return "Helvetica"
...
def _tbl(data, cw, fn, header_rows=1, sub_rows=None):
    ...
    cw_scaled = [w * 1.4 for w in cw] # 가로 40% 확대
...
def ledger_pdf(families_by_dept, period_text, df_src):
    ...
    for dept_name, grp_df in families_by_dept.items():
        ...
        E += [Paragraph("미처리 서류 확인서", st_["title"]), HRFlowable(width="100%",thickness=1.5,color=colors.HexColor(HDR_CLR)), Spacer(1,4),
              Paragraph(f"부서: {sec}  > {tg}  >  <b>{dept_name}</b>", st_["sub"]), Paragraph(f"적용기간: {period_text}  발급일자: {today}", st_["date"]), Spacer(1,6)]
        ...
        if not dept_src.empty:
            ...
            E.append(Spacer(1,8))
        E += [Paragraph(GUIDANCE_TEXT, st_["notice"]), Paragraph(PRECAUTION_TEXT_COVER, st_["notice"]), Spacer(1,8),
              Paragraph(f"작성일: {today}", st_["date"]), _sig_table(["부문장 확인","총괄 확인","부서장 확인"],fn,140), PageBreak()]
        for _, fam in grp_df.drop_duplicates("영업가족").iterrows():
            fam_name = fam["영업가족"]
            E += [Paragraph("서류 미처리 확인서", st_["title"]), HRFlowable(width="100%",thickness=1.5,color=colors.HexColor(HDR_CLR)), Spacer(1,4),
                  Paragraph(f"소속: {sec}  > {tg}  > {dept_name}  >  <b>{fam_name}</b>",st_["sub"]), Paragraph(f"적용기간: {period_text}  발급일자: {today}", st_["date"]), Spacer(1,6)]
            ...
            E += [Paragraph("▶ 양식별 미처리 요약", st_["section"]), _tbl(sum_d,[60,60,60,60],fn), Spacer(1,8),
                  ...
                  Paragraph("본인은 위 미처리 건수에 대하여 인지하였으며, 관련 리스크 관리에 최선을 다할 것을 확인합니다.", st_["body"]),
                  Paragraph(PRECAUTION_TEXT_SHEET, st_["notice"]), Spacer(1,10), Paragraph(f"작성일: {today}", st_["date"])]
...
def ledger_excel(families_by_dept, period_text, df_src):
    wb, ws0 = Workbook(), Workbook().active; ws0.title="목차"
    ...
    for dept_name,grp_df in families_by_dept.items():
        ...
        ws_c.merge_cells("A1:G1"); ws_c["A1"]=f"[{dept_name}]  미처리 서류 확인서"; ws_c["A1"].font=Font(name=tfn,size=14,bold=True)
        ws_c["A2"]=f"{sec}  > {tg}  > {dept_name}"; ws_c["A2"].font=Font(name=tfn,size=10)
        ws_c["A3"]=f"적용기간: {period_text}  |  발급일자: {today}"; ws_c["A3"].font=bf
        ...
        ws_c.cell(r,1,PRECAUTION_TEXT_COVER).font=nf; ws_c.cell(r,1).alignment=Alignment(wrapText=True); ws_c.row_dimensions[r].height=45; r+=3
        ...
        for _,fam in grp_df.drop_duplicates("영업가족").iterrows():
            ...
            ws_f=wb.create_sheet(title=fn_safe); ws_f.merge_cells("A1:G1"); ws_f["A1"]=f"[{fam_name}]  서류 미처리 확인서"; ws_f["A1"].font=Font(name=tfn,size=13,bold=True)
            ...
            ws_f.cell(r_f,1,PRECAUTION_TEXT_SHEET).font=nf; r_f+=2; ws_f.cell(r_f,1,f"작성일: {today}").font=bf; r_f+=1
...
def login_page():
    st.title("🔐 로그인")
    mode = st.radio("로그인 방식", ["관리자 로그인", "조직별 로그인"], horizontal=True)
    if mode == "관리자 로그인":
        user, pwd = st.text_input("관리자 아이디"), st.text_input("비밀번호", type="password")
        if st.button("로그인", use_container_width=True, type="primary"):
            if user in CREDENTIALS and CREDENTIALS[user]["password"] == pwd:
                st.session_state.logged_in, st.session_state.role = True, "admin"
                st.session_state.user_org, st.session_state.org_level = None, None
                st.rerun()
            else: st.error("아이디 또는 비밀번호 오류")
    else:
        org_name = st.text_input("조직명 입력 (부문 / 총괄 / 부서명)", placeholder="예: CA부문, 서울1총괄, 강남사업부")
        if st.button("조직으로 로그인", use_container_width=True):
            df = load_data()
            if df.empty: st.warning("데이터가 없습니다. 관리자에게 문의하세요.")
            else:
                level = None
                if org_name in df["부문"].dropna().unique(): level = "부문"
                elif org_name in df["총괄"].dropna().unique(): level = "총괄"
                elif org_name in df["부서"].dropna().unique(): level = "부서"
                if level:
                    st.session_state.logged_in, st.session_state.role = True, "user"
                    st.session_state.user_org, st.session_state.org_level = org_name, level
                    st.rerun()
                else: st.error("일치하는 조직명을 찾을 수 없습니다.")
...
def admin_page():
    ...
    with t1:
        ...
        if f:
            df_up = pd.read_excel(f); st.dataframe(df_up.head(5), use_container_width=True, hide_index=True)
            mode = st.radio("저장 방식", ["전체 대체","추가"], horizontal=True)
            if st.button("💾 저장", type="primary"):
                if upload_data(df_up, replace=(mode=="전체 대체")): st.success("✅ 저장 완료!")
    with t2:
        try:
            months = pd.read_sql("SELECT DISTINCT substr(보험시작일,1,7) as m FROM contracts ORDER BY m DESC", get_db())["m"].tolist()
            if months:
                sel = st.selectbox("삭제할 월", months); ok = st.checkbox(f"⚠️ {sel} 삭제에 동의합니다.")
...
def user_page():
    ...
    df = load_data()
    if df.empty: st.warning("📭 관리자가 데이터를 업로드해주세요."); return
    
    org_filter_msg = ""
    if st.session_state.get("role") == "user":
        org, level = st.session_state.get("user_org"), st.session_state.get("org_level")
        if org and level:
            if level == "부문": df = df[df["부문"] == org].copy()
            elif level == "총괄": df = df[df["총괄"] == org].copy()
            elif level == "부서": df = df[df["부서"] == org].copy()
            org_filter_msg = f"🔒 `{org}` ({level}) 권한 | 하위 데이터만 표시됨"
    ...
    try:
        ...
        st.info(f"📅 기준일: `{upd}` | 기간: `{period_text}` | 총 {len(df_sel):,}건 {org_filter_msg}")
    except: pass
    if df_sel.empty: st.info("선택한 기간/조직에 데이터가 없습니다."); return
    ...
    m1.metric("📄 총 계약건수", f"{len(df_sel):,}건"); m2.metric("⚠️ 총 미처리건수", f"{tot:,}건")
    m3.metric("📉 미처리율", f"{rate:.1f}%"); m4.metric("FA / 비교 / 완판", f"{fa_t} / {bi_t} / {cs_t}")
    st.divider()

    tab_dash, tab_report, tab_ledger = st.tabs(["📈 현황 대시보드", "📊 전체 계층 리포트", "📋 관리대장 선정 & 출력"])
    with tab_dash:
        cs1, cs2 = st.columns([2, 1])
        with cs1: search_text = st.text_input("🔍 조직 검색", placeholder="조직명 입력...")
        with cs2: agg_group = st.selectbox("집계 기준", ["부문","총괄","부서","영업가족"])
        agg = df_sel.groupby(agg_group).agg(FA고지_미스캔=("FA고지_c",_miss),비교설명_미스캔=("비교설명_c",_miss),완전판매_미스캔=("완전판매_c",_miss_cs),대상건=("증권번호","count")).reset_index()
    ...
    with tab_report:
        ...
        if report_df.empty: st.info("데이터가 없습니다.")
            ...
            with cr1:
                if st.button("📥 전체 계층 리포트 Excel", use_container_width=True):
            ...
            with cr2:
                if st.button("📥 전체 계층 리포트 PDF", use_container_width=True):
...
def main():
    ...
    if not st.session_state.get("logged_in"): login_page()
        ...
        with st.sidebar:
            ...
            if st.session_state.get("role") == "user":
                st.info(f"🏢 조직: `{st.session_state.get('user_org')}` ({st.session_state.get('org_level')})")
            if st.button("🚪 로그아웃", use_container_width=True):
                st.session_state.logged_in, st.session_state.role = False, None
                st.session_state.user_org, st.session_state.org_level = None, None
                st.rerun()
            st.divider()
            st.caption("v3.4 | 조직접근제어·클라우드배포최적화·표40%확대 | © 2026")

...
def load_data():
    ...
    try:
        ...
        df["개인정보_스캔"] = df["개인정보"].apply(is_scanned).astype(int) if "개인정보" in df.columns else 0
        df["개인정보_미스캔"] = df["개인정보"].apply(is_not_scanned).astype(int) if "개인정보" in df.columns else 0
...
def calculate_scan_stats(df_group):
    cnt = len(df_group)
    개인정보_대상 = cnt if "개인정보" in df_group.columns else 0
    FA고지_대상 = cnt
    비교설명_대상 = cnt
    개인정보_스캔 = int(df_group["개인정보_스캔"].sum()) if "개인정보" in df_group.columns else 0
    ...
    return {
        ...
        "전체_대상": 전체_대상, "전체_스캔": 전체_스캔, "전체_미스캔": 전체_미스캔, "스캔율": 스캔율,
    }
...
def build_group_scan_stats(df, group_col):
    ...
    # [수정] 미처리건수(총미스캔) 기준 내림차순 정렬
    return pd.DataFrame(rows).sort_values("총미스캔", ascending=False).reset_index(drop=True)
...
def build_hierarchy_report(df, months=None):
    ...
    for bm, df_bm in src.groupby("부문"):
        ...
        rows.append({"구분": "부문계", "부문": bm, "총괄": "", "부서": "", "영업가족": "",
                     "계약건수": stats["증번수"], "스캔대상건수": stats["전체_대상"], "전체스캔": stats["전체_스캔"],
                     "총미스캔": stats["전체_미스캔"], "스캔율": stats["스캔율"], "미처리율": round(100 - stats["스캔율"], 1)})
    ...
    # [수정] 미처리건수 기준 내림차순 정렬
    return report.sort_values("총미스캔", ascending=False).reset_index(drop=True)
    ...
    for mon, dm in src.groupby("월_피리어드"):
        for bm, db in dm.groupby("부문"):
            ...
            rows.append({"월": mon, "구분": "부문계", "부문": bm, "총괄": "", "부서": "",
                         ...
                         "계약건수": len(db), "미처리율": round((fa_b+bi_b+cs_b)/len(db)*100,1) if len(db) else 0.0})
            for tg, dt in db.groupby("총괄"):
                ...
                rows.append({"월": mon, "구분": "총괄계", "부문": bm, "총괄": tg, "부서": "",
                             ...
                             "계약건수": len(dt), "미처리율": round((fa_t+bi_t+cs_t)/len(dt)*100,1) if len(dt) else 0.0})
                for ds, dd in dt.groupby("부서"):
                    ...
                    rows.append({"월": mon, "구분": "부서계", "부문": bm, "총괄": tg, "부서": ds,
                                 ...
                                 "계약건수": cnt, "미처리율": round(tot/cnt*100,1) if cnt else 0.0})
...
def build_monthly_hierarchy_pivot(df, months=None):
    ...
    metrics = ["FA", "비교", "완판", "총미스캔", "계약건수", "미처리율"]
    pivot_frames = []
    ...
    return pivot[[c for c in ordered_columns if c in pivot.columns]].fillna(0)
...
def get_ledger_targets(df, months):
    ...
    agg = agg[agg["총미스캔"] > 0].sort_values("총미스캔", ascending=False)
...
def report_excel(df, months):
    ...
    # [수정] 용어 명확화 및 컬럼 추가
    headers = ["구분", "부문", "총괄", "부서", "영업가족", "계약건수", "스캔대상건수", "전체스캔", "총미스캔", "스캔율", "미처리율"]
    ...
    for _, row in report.iterrows(): 
        ...
        vals = [gbn, row["부문"], row["총괄"], row["부서"], row["영업가족"],
                row["계약건수"], row["스캔대상건수"], row["전체스캔"], row["총미스캔"],
        ...
        for ci,v in enumerate(vals,1):
            ...
            if isinstance(v,(int,float)): c.number_format = "#,##0" if ci < 9 else "0.0"
            if fill: c.fill=fill
        ri += 1
    ...
    if not monthly.empty:
        ...
        mhdr, mcws = ["월","구분","부문","총괄","부서","FA","비교","완판","총미스캔","계약건수","미처리율"], [18,14,20,20,24,12,12,12,14,12,16]
        ...
        for ri2,(_, r) in enumerate(monthly.iterrows(),3):
            ...
            vals2=[r["월"],gbn,r["부문"],r["총괄"],r["부서"],r["FA"],r["비교"],r["완판"],r["총미스캔"],r["계약건수"], rate_str]
...
def report_pdf(df, months):
    ...
    if not report.empty:
        ...
        hdr=[["구분","부문","총괄","부서","영업가족","계약건수","스캔대상건수","전체스캔","총미스캔","스캔율","미처리율"]]
        drows, sub_idx = [], []
        for i,(_,r) in enumerate(report.iterrows()):
            drows.append([r["구분"],r["부문"],r["총괄"],r["부서"],r["영업가족"],
                        f"{int(r['계약건수']):,}",f"{int(r['스캔대상건수']):,}",f"{int(r['전체스캔']):,}",
                        f"{int(r['총미스캔']):,}",f"{r['스캔율']:.1f}",f"{r['미처리율']:.1f}"])
            if r["구분"] in ("부문계","총괄계","부서계"): sub_idx.append(i+1)
        E.append(_tbl(hdr+drows,[24,32,32,32,45,22,28,22,26,22,22],fn,sub_rows=sub_idx)); E.append(Spacer(1,8))
    monthly = build_monthly_hierarchy(df, months)
    if not monthly.empty:
        ...
        mrows=[[r["월"],r["구분"],r["부문"],r["총괄"],r["부서"],
                ...
                f"{int(r['총미스캔']):,}",f"{int(r['계약건수']):,}",f"{r['미처리율']:.1f}"] for _,r in monthly.iterrows()]
        msub=[i+1 for i,(_,r) in enumerate(monthly.iterrows()) if r["구분"] in ("부문계","총괄계","부서계")]
        E.append(_tbl([["월","구분","부문","총괄","부서","FA","비교","완판","총미스캔","계약건수","미처리율"]]+mrows,[26,24,32,32,45,18,18,18,26,22,26],fn,sub_rows=msub))
...
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="그룹형", dash_top_n=15, map_type="🔲 트리맵"):
    ...
    doc = SimpleDocTemplate(buf, pagesize=landscape(A4), rightMargin=10*mm, leftMargin=10*mm, topMargin=10*mm, bottomMargin=10*mm)
    today, period_str = datetime.now().strftime("%Y년 %m월 %d일"), ", ".join(months) if months else "전체"
    E = [Paragraph("전체 페이지 요약 리포트", st_["title"]), Paragraph(f"기간: {period_str}  |  발급일자: {today}", st_["date"]), HRFlowable(width="100%", thickness=1, color=colors.HexColor(HDR_CLR)), Spacer(1, 6)]
    ...
    E.append(_tbl([
        ["총 계약건수", f"{stats_all['증번수']:,}"], ["스캔대상건수", f"{stats_all['전체_대상']:,}"],
        ["전체스캔", f"{stats_all['전체_스캔']:,}"], ["전체미스캔", f"{stats_all['전체_미스캔']:,}"],
        ["스캔율 / 미처리율", f"{stats_all['스캔율']:.1f} / {miss_rate:.1f}"],
    ], [90, 120], fn, header_rows=0, align="LEFT")); E.append(Spacer(1, 8))

    agg = build_group_scan_stats(df_sel, agg_group)
    if not agg.empty:
        E.append(Paragraph(f"▶ 현황 대시보드 집계 ({agg_group})", st_["section"]))
        hdr = [["조직", "계약건수", "스캔대상건수", "전체스캔", "총미스캔", "스캔율", "미처리율"]]
        rows = [[r["조직"], f"{int(r['계약건수']):,}", f"{int(r['스캔대상건수']):,}", f"{int(r['전체스캔']):,}", f"{int(r['총미스캔']):,}", f"{r['스캔율']:.1f}", f"{r['미처리율']:.1f}"] for _, r in agg.iterrows()]
        E.append(_tbl(hdr + rows, [80, 40, 52, 40, 40, 36, 36], fn)); E.append(Spacer(1, 8))
    ...
    if not map_agg.empty:
        ...
        hdr = [["조직", "계약건수", "스캔대상건수", "전체스캔", "미스캔", "스캔율", "미처리율"]]
        rows = [[r["조직"], f"{int(r['계약건수']):,}", f"{int(r['스캔대상건수']):,}", f"{int(r['전체스캔']):,}", f"{int(r['미스캔']):,}", f"{r['스캔율']:.1f}", f"{r['미처리율']:.1f}"] for _, r in map_agg.iterrows()]
        E.append(_tbl(hdr + rows, [80, 40, 52, 40, 40, 36, 36], fn)); E.append(Spacer(1, 8))

    report = build_hierarchy_report(df, months)
    if not report.empty:
        E.append(Paragraph("▶ 계층 리포트", st_["section"]))
        hdr = [["구분", "부문", "총괄", "부서", "영업가족", "계약건수", "스캔대상건수", "전체스캔", "총미스캔", "스캔율"]]
        drows = [[r["구분"], r["부문"], r["총괄"], r["부서"], r["영업가족"], f"{int(r['계약건수']):,}", f"{int(r['스캔대상건수']):,}", f"{int(r['전체스캔']):,}", f"{int(r['총미스캔']):,}", f"{r['스캔율']:.1f}"] for _, r in report.iterrows()]
        E.append(_tbl(hdr + drows, [30, 40, 40, 40, 55, 22, 28, 24, 24, 22], fn))
    doc.build(E); buf.seek(0); return buf
...
def dashboard_page():
    ...
    with tab_dash:
        ...
        if agg.empty: st.info("조건에 맞는 데이터가 없습니다.")
            ...
            st.dataframe(
                agg[["순위", "조직", "계약건수", "스캔대상건수", "전체스캔", "총미스캔", "스캔율", "미처리율", "FA고지_미스캔", "비교설명_미스캔", "완전판매_미스캔"]]
                .style.format({
                    "순위": "{:,}", "계약건수": "{:,}", "스캔대상건수": "{:,}", "전체스캔": "{:,}",
                    "총미스캔": "{:,}", "스캔율": "{:.1f}", "미처리율": "{:.1f}",
    ...
    with tab_map:
        ...
        if map_agg.empty: st.info("미처리 건수가 있는 데이터가 없습니다.")
            ...
            st.dataframe(map_agg[["조직","계약건수","스캔대상건수","전체스캔","미스캔","스캔율","미처리율"]].style.format({
                "계약건수":"{:,}","스캔대상건수":"{:,}","전체스캔":"{:,}","미스캔":"{:,}","스캔율":"{:.1f}","미처리율":"{:.1f}"
            }), use_container_width=True, hide_index=True)
    ...
    with tab_report:
        ...
        if report_df.empty: st.info("데이터가 없습니다.")
            ...
            def style_row(row):
                ...
                return [""]*len(row)
            disp_cols = ["구분","부문","총괄","부서","영업가족","계약건수","스캔대상건수","전체스캔","총미스캔","스캔율","미처리율"]
            disp_df = report_df[disp_cols].copy()
            st.markdown("""<style>table { white-space: nowrap !important; font-size: 0.85rem !important; }.stDataFrame { width: 100% !important; overflow-x: auto !important; }table td, table th { padding: 3px 5px !important; line-height: 1.2 !important; border-bottom: 1px solid #ddd; }</style>""", unsafe_allow_html=True)
            st.caption("스캔 처리: 스캔, M스캔, 보험사스캔 / 완판대상: 스캔, M스캔, 미스캔만 대상")
            st.dataframe(disp_df.style.apply(style_row, axis=1).format({
                "계약건수":"{:,}","스캔대상건수":"{:,}","전체스캔":"{:,}","총미스캔":"{:,}","스캔율":"{:.1f}","미처리율":"{:.1f}"
            ...
            if not monthly_df.empty:
                st.dataframe(monthly_df.style.format({"FA":"{:,}","비교":"{:,}","완판":"{:,}","총미스캔":"{:,}","계약건수":"{:,}","미처리율":"{:.1f}"}), use_container_width=True, hide_index=True, height=420)
...
def main():
    if not st.session_state.get("logged_in"): login_page()
        ...
        with st.sidebar:
            ...
            st.divider()
            st.caption("v5.2 | 용어정확화·미스캔정렬·오타수정완료 | © 2026")
        dashboard_page()

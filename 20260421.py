...
GUIDANCE_TEXT = (
    ...
    "상기 서류는 소비자 보호 및 설명 의무 이행 여부를 확인하기 위한 내부 통제 관리 대상 서류로서, "
    "실적 확정 입력 마감 시점까지 제출 완료를 원칙으로 하며 미비 시 내부 통제 리스크 관리 대상 계약으로 분류됩니다."
)
PRECAUTION_TEXT_COVER = (
    "【미처리 시 유의사항】\n"
    "실적 확정 입력 마감 시점까지 필수 서류가 제출되지 않은 계약과 조직에 대하여는 "
    "모집질서 및 분쟁 리스크 관리 대상으로 분류되어 관리됩니다.\n"
...
REQUIRED_DOCS_TABLE = [
    ...
    ["4 ", "완전판매확인서\n(대상: 종신, CI, CEO경기, 고액) ", "금융소비자보호법 제17·19조 설명 적합성 적정성 관련 조항\n영업지원기준안 ", "약관,청약서 부본 제공, 중요 상품 이해 및\n자발적 가입 확인, 설명 의무 이행 증빙력 확보 "]
]

# ==========================================
# 2. 데이터 로딩 (GitHub 엑셀 기반) ✅ 집계 로직 교체됨
# ==========================================
...
def load_data():
    ...
    try:
        ...
        df["완전판매_미스캔"] = df["완전판매_c"].apply(lambda x: is_cs_target(x) and not is_scanned(x)).astype(int)
...
def calculate_scan_stats(df_group):
    ...
    # ✅ 스캔율 (기존 "미처리율" → "스캔율"로 변경)
    스캔율 = round((전체_스캔 / 전체_대상 * 100), 1) if 전체_대상 > 0 else 0.0
    
    return {
        "증번수": cnt,
        "개인정보_대상": 개인정보_대상, "개인정보_스캔": 개인정보_스캔, "개인정보_미스캔": 개인정보_대상 - 개인정보_스캔,
        "FA고지_대상": FA고지_대상, "FA고지_스캔": FA고지_스캔, "FA고지_미스캔": FA고지_대상 - FA고지_스캔,
        "비교설명_대상": 비교설명_대상, "비교설명_스캔": 비교설명_스캔, "비교설명_미스캔": 비교설명_대상 - 비교설명_스캔,
        "완전판매_대상": 완전판매_대상, "완전판매_스캔": 완전판매_스캔, "완전판매_미스캔": 완전판매_대상 - 완전판매_스캔,
        "전체_대상": 전체_대상, "전체_스캔": 전체_스캔, "전체_미스캔": 전체_미스캔,
        "스캔율": 스캔율
    }

# ==========================================
# 4. 전체 계층 리포트 ✅ 스캔율 기반 집계로 교체
# ==========================================
@st.cache_data(ttl=300)
def build_hierarchy_report(df, months=None):
    ...
    for bm, df_bm in src.groupby("부문"):
        ...
        rows.append({
            ...
            "증번수": stats["증번수"],
            "개인정보": f"{stats['개인정보_스캔']}/{stats['개인정보_대상']}",
            "FA고지": f"{stats['FA고지_스캔']}/{stats['FA고지_대상']}",
            "비교설명": f"{stats['비교설명_스캔']}/{stats['비교설명_대상']}",
            "완전판매": f"{stats['완전판매_스캔']}/{stats['완전판매_대상']}",
            "전체대상": stats["전체_대상"], "전체스캔": stats["전체_스캔"], "전체미스캔": stats["전체_미스캔"],
            "스캔율": stats["스캔율"]
        })
...
def build_monthly_hierarchy(df, months=None):
    ...
    for mon, dm in src.groupby("월_피리어드"):
        for bm, db in dm.groupby("부문"):
            ...
            rows.append({
                "월": mon, "구분": "부문계", "부문": bm, "총괄": " ", "부서": " ",
                "증번수": stats["증번수"], "전체대상": stats["전체_대상"], "전체스캔": stats["전체_스캔"], "전체미스캔": stats["전체_미스캔"],
                "스캔율": stats["스캔율"]
            })
        for tg, dt in db.groupby("총괄"):
            stats = calculate_scan_stats(dt)
            rows.append({
                "월": mon, "구분": "총괄계", "부문": bm, "총괄": tg, "부서": " ",
                "증번수": stats["증번수"], "전체대상": stats["전체_대상"], "전체스캔": stats["전체_스캔"], "전체미스캔": stats["전체_미스캔"],
                "스캔율": stats["스캔율"]
            })
            for ds, dd in dt.groupby("부서"):
                ...
                rows.append({
                    "월": mon, "구분": "부서계", "부문": bm, "총괄": tg, "부서": ds,
                    "증번수": stats["증번수"], "전체대상": stats["전체_대상"], "전체스캔": stats["전체_스캔"], "전체미스캔": stats["전체_미스캔"],
                    "스캔율": stats["스캔율"]
                })
    return pd.DataFrame(rows)
...
def get_ledger_targets(df, months):
    ...
    if src.empty: return {}
    # 미스캔 합계
    src["총_미스캔"] = src["개인정보_미스캔"] + src["FA고지_미스캔"] + src["비교설명_미스캔"] + src["완전판매_미스캔"]
    ...
    agg["총미스캔"] = agg[["개인정보", "FA고지", "비교설명", "완판"]].sum(axis=1)
    agg = agg[agg["총미스캔"] > 0]
...
# ==========================================
# 7. 전체 계층 리포트 Excel ✅ 스캔율 기반 라벨 교체
# ==========================================
def report_excel(df, months):
    wb = Workbook(); ws = wb.active; ws.title="계층별_스캔현황"
    ...
    ws.merge_cells("A1:K1"); ws["A1"] = f"서류 스캔 현황 계층별 집계  ·  기간: {period_str}  ·  발급: {today}"
    ws["A1"].font = Font(name=tfn,size=12,bold=True); ws["A1"].alignment = Alignment(horizontal="center"); ws.row_dimensions[1].height = 22
    headers = ["구분", "부문", "총괄", "부서", "영업가족", "증번수", "개인정보", "FA고지", "비교설명", "완전판매", "전체대상", "전체스캔", "전체미스캔", "스캔율"]
    cws = [10,14,14,14,16,8,12,10,10,10,10,10,10,8]
    ...
    for _, row in report.iterrows(): 
        ...
        vals = [gbn, row["부문"], row["총괄"], row["부서"], row["영업가족"], row["증번수"], 
                row["개인정보"], row["FA고지"], row["비교설명"], row["완전판매"],
    ...
    if not monthly.empty:
        ...
        ws2.merge_cells("A1:K1"); ws2["A1"] = f"월별 계층 스캔 집계  ·  기간: {period_str}  ·  발급일: {today}"
        ...
        mcws = [14,10,14,14,16,8,10,10,10,8]
...
def report_pdf(df, months):
    ...
    today, period_str = datetime.now().strftime("%Y년 %m월 %d일"), ", ".join(months) if months else "전체"
    E = [Paragraph("서류 스캔 현황 계층별 집계", st_["title"]), Paragraph(f"기간: {period_str}  |  발급일자: {today}", st_["date"]), HRFlowable(width="100%",thickness=1,color=colors.HexColor(HDR_CLR)), Spacer(1,6)]
    report = build_hierarchy_report(df, months)
    if not report.empty:
        E.append(Paragraph("▶ 부문 / 총괄 / 부서 / 영업가족 계층 집계", st_["section"]))
        hdr=[["구분", "부문", "총괄", "부서", "영업가족", "증번수", "개인정보", "FA고지", "비교설명", "완전판매", "전체대상", "전체스캔", "전체미스캔", "스캔율"]]
        drows, sub_idx = [], []
        for i,(_,r) in enumerate(report.iterrows()):
            drows.append([r["구분"],r["부문"],r["총괄"],r["부서"],r["영업가족"],r["증번수"],r["개인정보"],r["FA고지"],r["비교설명"],r["완전판매"],r["전체대상"],r["전체스캔"],r["전체미스캔"],f"{r['스캔율']}%"])
            if r["구분"] in ("부문계", "총괄계", "부서계"): sub_idx.append(i+1)
        cw=[15,25,25,25,35,15,20,18,20,18,18,18,18,18]
        E.append(_tbl(hdr+drows,cw,fn,sub_rows=sub_idx)); E.append(Spacer(1,8))
    monthly = build_monthly_hierarchy(df, months)
    if not monthly.empty:
        E.append(PageBreak()); E.append(Paragraph("▶ 월별 계층별 스캔 집계", st_["section"]))
    ...
    for dept_name, grp_df in families_by_dept.items():
        ...
        if not dept_src.empty:
            ...
            fam_mon = dept_src.groupby(["영업가족", "월_피리어드"]).agg(
                개인정보=("개인정보_미스캔", "sum"), FA=("FA고지_미스캔", "sum"),비교=("비교설명_미스캔", "sum"),완판=("완전판매_미스캔", "sum")
            ).reset_index()
            fam_mon["계"] = fam_mon[["개인정보", "FA", "비교", "완판"]].sum(axis=1)
            fam_mon = fam_mon[fam_mon["계"] > 0]
            if not fam_mon.empty:
                td=[["영업가족", "월", "개인정보", "FA고지", "비교설명", "완전판매", "계"]]
                for _, r in fam_mon.iterrows(): td.append([r["영업가족"],r["월_피리어드"],int(r.개인정보),int(r.FA),int(r.비교),int(r.완판),int(r["계"])])
        ...
        for _, fam in grp_df.drop_duplicates("영업가족").iterrows():
            ...
            sosok["계"] = sosok[["개인정보", "FA", "비교", "완판"]].sum(axis=1)
            sosok = sosok[sosok["계"] > 0]
            E.append(Paragraph("▶ 소속별 · 월별 · 양식별 미처리 건수", section_left))
            if not sosok.empty:
                td2=[["소속", "월", "개인정보", "FA고지", "비교설명", "완전판매", "계"]]
                for _,r in sosok.iterrows(): td2.append([r["소속"],r["월_피리어드"],int(r.개인정보),int(r.FA),int(r.비교),int(r.완판),int(r["계"])])
                E.append(_tbl(td2,[130,50,45,45,45,45],fn, align="LEFT"))
            E.append(Spacer(1,4))
            sum_d=[["개인정보", "FA고지", "비교설명", "완전판매", "총계"],
                   [str(int(fam["개인정보"])),str(int(fam["FA"])),str(int(fam["비교"])),str(int(fam["완판"])),str(int(fam["총미스캔"]))]]
...
def ledger_excel(families_by_dept, period_text, df_src):
    ...
    for dept,grp in families_by_dept.items():
        for _,fam in grp.drop_duplicates("영업가족").iterrows():
            for ci,v in enumerate([dept,fam["영업가족"],int(fam["개인정보"]),int(fam["FA"]),int(fam["비교"]),int(fam["완판"]),int(fam["총미스캔"])],1):
    ...
    for dept_name,grp_df in families_by_dept.items():
        ...
        dept_src=df_src[df_src["부서"]==dept_name]
        if not dept_src.empty:
            fam_mon=dept_src.groupby(["영업가족", "월_피리어드"]).agg(개인정보=("개인정보_미스캔", "sum"),FA=("FA고지_미스캔", "sum"),비교=("비교설명_미스캔", "sum"),완판=("완전판매_미스캔", "sum")).reset_index()
            fam_mon["계"]=fam_mon[["개인정보", "FA", "비교", "완판"]].sum(axis=1); fam_mon=fam_mon[fam_mon["계"] > 0]
            ...
            for i,(_,rv) in enumerate(fam_mon.iterrows()):
                row_v=[rv["영업가족"],rv["월_피리어드"],int(rv.개인정보),int(rv.FA),int(rv.비교),int(rv.완판),int(rv["계"])]
        ...
        for _,fam in grp_df.drop_duplicates("영업가족").iterrows():
            ...
            fam_src=df_src[(df_src["영업가족"]==fam_name) & df_src["소속"].notna()]
            sosok=fam_src.groupby(["소속", "월_피리어드"]).agg(개인정보=("개인정보_미스캔", "sum"),FA=("FA고지_미스캔", "sum"),비교=("비교설명_미스캔", "sum"),완판=("완전판매_미스캔", "sum")).reset_index()
            sosok["계"]=sosok[["개인정보", "FA", "비교", "완판"]].sum(axis=1); sosok=sosok[sosok["계"] > 0]
            ...
            if not sosok.empty:
                for i,(_,sr) in enumerate(sosok.iterrows()):
                    rv2=[sr["소속"],sr["월_피리어드"],int(sr.개인정보),int(sr.FA),int(sr.비교),int(sr.완판),int(sr["계"])]; af=alt_fill if i%2==1 else None
            ...
            for ci,v in enumerate([int(fam["개인정보"]),int(fam["FA"]),int(fam["비교"]),int(fam["완판"]),int(fam["총미스캔"])],1): c=ws_f.cell(r_f+1,ci,v); c.font=bf; c.border=bdr; c.alignment=Alignment(horizontal="center")
            r_f+=3
...
# ==========================================
# 12. 통합 대시보드 ✅ 스캔율 기반 KPI/차트 라벨 교체
# ==========================================
def dashboard_page():
    ...
    m3.metric("📉 스캔율", f"{스캔율}%")
    m4.metric("개인정보/FA/비교/완판 미스캔", f"{int(df_sel['개인정보_미스캔'].sum())} / {int(df_sel['FA고지_미스캔'].sum())} / {int(df_sel['비교설명_미스캔'].sum())} / {int(df_sel['완전판매_미스캔'].sum())}")
    st.divider()

    tab_dash, tab_report, tab_ledger = st.tabs([
         "📈 현황 대시보드",  "📊 계층 리포트",  "📋 관리대장 출력"
    ])
    ...
    with tab_dash:
        ...
        if not agg.empty:
            ...
            agg = agg.sort_values("스캔율", ascending=True).reset_index(drop=True)
            agg.insert(0, "순위", range(1, len(agg)+1))
            
            st.dataframe(
                agg[["순위", "조직", "증번수", "전체대상", "전체스캔", "전체미스캔", "스캔율"]]
                .style.format({"증번수": "{:,}", "전체대상": "{:,}", "전체스캔": "{:,}", "전체미스캔": "{:,}", "스캔율": "{:.1f}%"}),
    ...
    with tab_report:
        st.subheader("📊 전체 데이터 기반 계층별 스캔 현황")
        ...
        if report_df.empty: st.info("데이터가 없습니다.")
            ...
            if not pivot_df.empty:
                ...
                pivot_display["스캔율"] = pivot_display["스캔율"].apply(lambda x: f"{x:.1f}%")
                st.dataframe(pivot_display.style.format({"증번수": "{:,}", "전체대상": "{:,}", "전체스캔": "{:,}", "전체미스캔": "{:,}"}), use_container_width=True, hide_index=True, height=420)
    ...
    with tab_ledger:
        ...
        if not targets: st.success("✅ 미스캔 발생 대상이 없습니다.")
            ...
            prev = [{"부문":r["부문"], "총괄":r["총괄"], "부서":dept, "영업가족":r["영업가족"], "개인정보":int(r["개인정보"]), "FA":int(r["FA고지"]), "비교":int(r["비교설명"]), "완판":int(r["완판"]), "총미스캔":int(r["총미스캔"])} for dept, grp in targets.items() for _, r in grp.iterrows()]
            prev_df = pd.DataFrame(prev)
            st.markdown(f"#### 📌 선정 대상 — 총 **{len(prev_df)}** 개 영업가족")
            st.dataframe(prev_df.style.format({"개인정보": "{:,}", "FA": "{:,}", "비교": "{:,}", "완판": "{:,}", "총미스캔": "{:,}"}), use_container_width=True, hide_index=True)

...
# ==========================================
# GitHub에 업로드할 엑셀 파일명
EXCEL_FILE = "insurance_data.xlsb"  # ✅ xlsb 지원
...
def load_data():
    ...
    try:
        ...
        # ✅ 1. 열 이름 앞뒤 공백/탭 제거 (엑셀 내보내기 시 흔히 발생)
        df.columns = df.columns.str.strip()
        
        # ✅ 2. 월도(날짜) 열 자동 탐지
        date_col = None
        for col in df.columns:
            if "보험시작일" in col or "시작일" in col or "계약일" in col:
                date_col = col
                break
        
        # 이름으로 못 찾으면 H열(인덱스 7) 자동 사용
        if date_col is None:
            if len(df.columns) > 7:
                date_col = df.columns[7]
                st.warning(f"⚠️ '보험시작일' 열을 찾지 못해 H열('{date_col}')을 월도 기준으로 자동 적용합니다.")
            else:
                st.error("❌ 월도(날짜) 열을 찾을 수 없습니다. 열 이름 또는 위치를 확인해주세요.")
                return pd.DataFrame()

        # 날짜 파싱 및 월 피리어드 생성
        df["보험시작일_dt"] = pd.to_datetime(df[date_col], errors="coerce")
        df["월_피리어드"] = df["보험시작일_dt"].dt.to_period("M").astype(str)
        st.success(f"✅ 월도 기준 열: '{date_col}' | 총 {len(df):,}건 로드")
        
        # 날짜 및 필드 전처리 (원본 로직 유지, 공백 제거된 기준 적용)
...
def ledger_pdf(families_by_dept, period_text, df_src):
    ...
    # ✅ SyntaxError 해결: 12mm → 12*mm, 15mm → 15*mm

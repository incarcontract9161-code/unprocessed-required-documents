...
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="group", dash_top_n=15, map_type="treemap", view_mode="조직별 보기", show_breakdown=False):
    ...
    # ══════════════════════════════════════════════════════════════════
    elements.append(PageBreak())
    elements.append(Paragraph(f"대시보드 차트 — {agg_group} 기준 / {period_str}", st_["title"]))
    ...
    try:
        elements.append(Paragraph(f"② {agg_group} 기준 월별 미처리건수 & 스캔율 추이", st_["section"]))
        ...
        fig_mon.update_layout(
            title=f"{agg_group} 기준 월별 미처리건수 & 스캔율 ({period_str})",
        ...
        fig_mon.update_xaxes(tickangle=-30)
    ...
    try:
        elements.append(Paragraph(f"③ {agg_group} 기준 서류별 월간 미처리건수 추이", st_["section"]))
        fig_doc = go.Figure()
        for col_key, lbl, clr in [("FA_miss","FA고지","#2E86C1"),("비교_miss","비교설명","#8E44AD"),("완판_miss","완전판매","#27AE60")]:
        ...
        fig_doc.update_layout(
            title=f"{agg_group} 기준 서류별 월간 미처리건수 추이 ({period_str})",
    ...
    with tab_dash:
        ...
        if agg.empty:
            ...
            # ══════════════════════════════════════════════════
                ...
                st.markdown(f"#### 📅 월별 미처리 추이 — 집계 기준: **{agg_group}** | 선택 월: {', '.join(sel_months)}")
                ...
                # [추가] 월별 트렌드 집계 후 차트 렌더링
                df_trend = build_monthly_trend_data(df_sel, month_filter or sel_months, agg_group)
                render_monthly_trend_chart(df_trend, agg_group, show_doc_breakdown=show_breakdown)

                # 검색 조직 한정 추이 (검색어 있을 때만)
                if search_text and not agg.empty:
                    st.markdown("---")
                    st.markdown(f"#### 🔍 검색 조직 한정 월별 추이: `{search_text}`")
                    ...
                    if not df_search_src.empty:
                        ...
                        render_monthly_trend_chart(df_trend_search, f"{agg_group} · {search_text}", show_doc_breakdown=show_breakdown)
                    else:

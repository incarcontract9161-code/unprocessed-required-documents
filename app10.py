...
def login_page():
    st.title("🔐 시스템 접속")
    st.markdown("관리자/사용자 구분 없이 **단일 비밀번호**로 접속합니다.")
...
# ==========================================
# 13. 통합 대시보드 (관리자+사용자 기능 통합)
# ==========================================
def dashboard_page():
    st.title("📊 서류 처리 현황 대시보드")
    df = load_data()
    if df.empty:
        st.warning("📭 데이터가 없습니다. '데이터 관리' 탭에서 엑셀을 업로드해주세요."); return
    ...
    try:
        ...
        st.info(f"📅 기준일: `{upd}` | 기간: `{period_text}` | 총 {len(df_sel):,}건")
    except: pass
    ...
    tab_mgmt, tab_dash, tab_map, tab_report, tab_ledger = st.tabs([
        "📤 데이터 관리", "📈 현황 대시보드", "🗺️ 미처리맵", "📊 계층 리포트", "📋 관리대장 출력"
    ])

    # ── TAB 1 : 데이터 관리 ─────────────────────────
    with tab_mgmt:
        t1,t2 = st.tabs(["📥 엑셀 업로드", "🗑️ 월별 삭제"])
        with t1:
            ...
            if f:
                ...
                if st.button("💾 저장", type="primary"):
                    if upload_data(df_up, replace=(mode=="전체 대체")): st.success("✅ 저장 완료! 데이터가 갱신되었습니다.")
        with t2:
            try:
    ...
    if not st.session_state.get("logged_in"):
        ...
        with st.sidebar:
            ...
            if st.button("🚪 로그아웃", use_container_width=True):
                ...
                st.rerun()
            st.divider()
            st.caption("v4.0 | 단일비밀번호·기능통합·배포최적화 | © 2026")
        dashboard_page()

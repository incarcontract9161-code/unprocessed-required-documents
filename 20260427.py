...
def calculate_scan_stats(df_group):
    cnt = len(df_group)
    개인정보_대상 = cnt; FA고지_대상 = cnt; 비교설명_대상 = cnt
    개인정보_스캔 = int(df_group["개인정보_스캔"].sum()) if "개인정보_스캔" in df_group.columns else 0
    FA고지_스캔 = int(df_group["FA고지_스캔"].sum()) if "FA고지_스캔" in df_group.columns else 0
    비교설명_스캔 = int(df_group["비교설명_스캔"].sum()) if "비교설명_스캔" in df_group.columns else 0
    완전판매_대상 = int(df_group["완전판매_대상"].sum()) if "완전판매_대상" in df_group.columns else 0
    완전판매_스캔 = int(df_group["완전판매_스캔"].sum()) if "완전판매_스캔" in df_group.columns else 0
    ...
    if not report.empty:
        ...
        for i,(_,r) in enumerate(report.iterrows()):
            drows.append([r["구분"],r["부문"],r["총괄"],r["부서"],r["영업가족"],
                        f"{int(r['증번수']):,}",f"{int(r['전체대상']):,}",f"{int(r['전체스캔']):,}",
                        f"{int(r['전체미스캔']):,}",f"{r['스캔율']:.1f}%"])
    ...
    if not st.session_state.get("logged_in"):
        ...
        with st.sidebar:
            ...
            if st.button("🚪 로그아웃", use_container_width=True):
                ...
                st.rerun()
            st.divider()
            st.caption("v5.1 | GitHub엑셀기반·실시간배포 | © 2026")
        dashboard_page()

...
DATA_REFERENCE_DATE = os.environ.get("DATA_REFERENCE_DATE", datetime.now().strftime("%Y-%m-%d"))
...
PRECAUTION_TEXT_COVER = (
    ...
    "신계약 리스크 점검 강화 및 지속적인 개선이 이루어지지 않을 경우, 신규 지원금 승인이 보류되거나 참여 요건이 제한될 수 있습니다."
)
...
def build_hierarchy_report(df, months=None):
    ...
    for bm, df_bm in src.groupby("부문"):
        ...
        rows.append({"구분": "부문계", "부문":bm, "총괄": " ", "부서": " ", "영업가족": " ", "FA":fa, "비교":bi, "완판":cs, "총미스캔":tot, "대상건":cnt, "미처리율":round(tot/cnt*100,1) if cnt else 0.0})
        for tg, df_tg in df_bm.groupby("총괄"):
            ...
            rows.append({"구분": "총괄계", "부문":bm, "총괄":tg, "부서": " ", "영업가족": " ", "FA":fa2, "비교":bi2, "완판":cs2, "총미스캔":tot2, "대상건":cnt2, "미처리율":round(tot2/cnt2*100,1) if cnt2 else 0.0})
            for ds, df_ds in df_tg.groupby("부서"):
                ...
                rows.append({"구분": "부서계", "부문":bm, "총괄":tg, "부서":ds, "영업가족": " ", "FA":fa3, "비교":bi3, "완판":cs3, "총미스캔":tot3, "대상건":cnt3, "미처리율":round(tot3/cnt3*100,1) if cnt3 else 0.0})
                for fg, df_fg in df_ds.groupby("영업가족"):
                    ...
                    rows.append({"구분": "영업가족", "부문":bm, "총괄":tg, "부서":ds, "영업가족":fg, "FA":fa4, "비교":bi4, "완판":cs4, "총미스캔":t4, "대상건":c4, "미처리율":round(t4/c4*100,1) if c4 else 0.0})
...
def build_monthly_hierarchy(df, months=None):
    ...
    for mon, dm in src.groupby("월_피리어드"):
        for bm, db in dm.groupby("부문"):
            ...
            rows.append({"월":mon, "구분": "부문계", "부문":bm, "총괄": " ", "부서": " ", "FA":fa_b, "비교":bi_b, "완판":cs_b, "총미스캔":fa_b+bi_b+cs_b, "대상건":len(db), "미처리율":round((fa_b+bi_b+cs_b)/len(db)*100,1) if len(db) else 0.0})
        for tg, dt in db.groupby("총괄"):
            fa_t = int(dt["FA_miss"].sum()); bi_t = int(dt["비교_miss"].sum()); cs_t = int(dt["완판_miss"].sum())
            rows.append({"월":mon, "구분": "총괄계", "부문":bm, "총괄":tg, "부서": " ", "FA":fa_t, "비교":bi_t, "완판":cs_t, "총미스캔":fa_t+bi_t+cs_t, "대상건":len(dt), "미처리율":round((fa_t+bi_t+cs_t)/len(dt)*100,1) if len(dt) else 0.0})
            for ds, dd in dt.groupby("부서"):
                fa = int(dd["FA_miss"].sum()); bi = int(dd["비교_miss"].sum()); cs = int(dd["완판_miss"].sum())
                tot = fa + bi + cs; cnt = len(dd)
                rows.append({"월":mon, "구분": "부서계", "부문":bm, "총괄":tg, "부서":ds, "FA":fa, "비교":bi, "완판":cs, "총미스캔":tot, "대상건":cnt, "미처리율":round(tot/cnt*100,1) if cnt else 0.0})
    return pd.DataFrame(rows)
...
def get_ledger_targets(df, months):
    ...
    if src.empty: return {}
    agg = src.groupby(["부문", "총괄", "부서", "영업가족"]).agg(FA=("FA_miss", "sum"), 비교=("비교_miss", "sum"), 완판=("완판_miss", "sum"), 대상=("증권번호", "count")).reset_index()
...
def report_excel(df, months):
    ...
    fills = {"부문계":PatternFill("solid",fgColor="1F3864"), "총괄계":PatternFill("solid",fgColor="2E75B6"), "부서계":PatternFill("solid",fgColor="D9E1F2"), "영업가족_alt":PatternFill("solid",fgColor="EEF3FB")}
    fonts_wc = {"부문계":Font(name=tfn,size=9,bold=True,color="FFFFFF"), "총괄계":Font(name=tfn,size=9,bold=True,color="FFFFFF"), "부서계":Font(name=tfn,size=9,bold=True)}
...
def report_pdf(df, months):
    ...
    if not report.empty:
        ...
        for i,(_,r) in enumerate(report.iterrows()):
            drows.append([r["구분"],r["부문"],r["총괄"],r["부서"],r["영업가족"],f"{int(r['FA']):,}",f"{int(r['비교']):,}",f"{int(r['완판']):,}",f"{int(r['총미스캔']):,}",f"{int(r['대상건']):,}",f"{r['미처리율']:.1f}%"])
    ...
    if not monthly.empty:
        ...
        mrows=[[r["월"],r["구분"],r["부문"],r["총괄"],r["부서"],f"{int(r['FA']):,}",f"{int(r['비교']):,}",f"{int(r['완판']):,}",f"{int(r['총미스캔']):,}",f"{int(r['대상건']):,}",f"{r['미처리율']:.1f}%"] for _,r in monthly.iterrows()]
    ...
    if not pivot.empty:
        ...
        E.append(_tbl(headers + values, cw, fn))
...
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="그룹형", dash_top_n=15, map_type="🔲 트리맵"):
    ...
    if not agg.empty:
        ...
        try:
            if len(dash_doc_types)==1 and dash_doc_types[0]=="총 미스캔":
                ...
                fig_dash.update_layout(xaxis_tickangle=-45, height=340)
            append_pdf_figure(E, fig_dash, st_, max_width=1000, height=340)
        except Exception as e:
        ...
        try:
            ...
            append_pdf_figure(E, fig_trend, st_, max_width=1000, height=340)
        except Exception as e:
    ...
    if not map_agg.empty:
        ...
        try:
            ...
            append_pdf_figure(E, fig_map, st_, max_width=1000, height=340)
        except Exception as e:
...
def dashboard_page():
    ...
    with col2:
        st.info(f"📅 기준: **{DATA_REFERENCE_DATE}**")
    with col3:
        if st.button("🔄 새로고침"):
            st.cache_data.clear()
            st.rerun()
    ...
    with tab_dash:
        ...
        if agg.empty: st.info("조건에 맞는 데이터가 없습니다.")
            ...
            st.dataframe(agg[["순위", "조직", "총_미스캔", "미처리율", "FA고지_미스캔", "비교설명_미스캔", "완전판매_미스캔"]].style.format({"순위": "{:,}", "총_미스캔": "{:,}", "미처리율": "{:.1f}%", "FA고지_미스캔": "{:,}", "비교설명_미스캔": "{:,}", "완전판매_미스캔": "{:,}"}), use_container_width=True, hide_index=True)
    ...
    with tab_report:
        ...
        if report_df.empty: st.info("데이터가 없습니다.")
            ...
            st.dataframe(disp_df.style.apply(style_row, axis=1).format({"FA": "{:,}", "비교": "{:,}", "완판": "{:,}", "총미스캔": "{:,}", "대상건": "{:,}", "미처리율": "{:.1f}%"}), use_container_width=True, hide_index=True, height=500)
            st.divider()
    ...
    if not st.session_state.get("logged_in"):
        ...
        with st.sidebar:
            ...
            if st.button("🚪 로그아웃", use_container_width=True):
                ...
                st.rerun()
            st.divider()
            st.caption(f"v5.0 | 기준일:{DATA_REFERENCE_DATE} | © 2026")
        dashboard_page()

...
# ==========================================
# 1. 전역 설정 (✅ 배포 전 이곳에서 설정하세요!)
# ==========================================
EXCEL_FILE = "insurance_data.xlsx"

# 🔑 비밀번호 설정 (환경변수 우선, 없으면 기본값 사용)
APP_PASSWORD = os.environ.get("APP_PASSWORD", "incar961")

# 📅 데이터 기준일 설정 (배포할 때마다 이곳을 수정하세요)
DATA_REFERENCE_DATE = "2026-04-13" 
...
def report_excel(df, months):
    ...
    ws.merge_cells("A1:K1"); ws["A1"] = f"서류 미처리 현황 계층별 집계  ·  기간: {period_str}  ·  기준일: {DATA_REFERENCE_DATE}"
    ...
    if not monthly.empty:
        ...
        ws2.merge_cells("A1:K1"); ws2["A1"] = f"월별 계층 미처리 집계  ·  기간: {period_str}  ·  기준일: {DATA_REFERENCE_DATE}"
    ...
    if not pivot.empty:
        ...
        ws3.merge_cells("A1:Z1"); ws3["A1"] = f"월별 피벗형 계층 집계  ·  기간: {period_str}  ·  기준일: {DATA_REFERENCE_DATE}"
...
def report_pdf(df, months):
    ...
    period_str = ", ".join(months) if months else "전체"
    E = [Paragraph("서류 미처리 현황 계층별 집계", st_["title"]), Paragraph(f"기간: {period_str}  |  기준일: {DATA_REFERENCE_DATE}", st_["date"]), HRFlowable(width="100%",thickness=1,color=colors.HexColor(HDR_CLR)), Spacer(1,6)]
    report = build_hierarchy_report(df, months)
    if not report.empty:
...
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="그룹형", dash_top_n=15, map_type="🔲 트리맵"):
    ...
    period_str = ", ".join(months) if months else "전체"
    E = [Paragraph("전체 페이지 요약 리포트", st_["title"]), Paragraph(f"기간: {period_str}  |  기준일: {DATA_REFERENCE_DATE}", st_["date"]), HRFlowable(width="100%",thickness=1,color=colors.HexColor(HDR_CLR)), Spacer(1,8)]
...
def ledger_pdf(families_by_dept, period_text, df_src):
    ...
    for dept_name, grp_df in families_by_dept.items():
        ...
        E += [Paragraph(PRECAUTION_TEXT_COVER, notice_left), Spacer(1,4),
              ...
              Paragraph(f"기준일: {DATA_REFERENCE_DATE}", center_date_style), Spacer(1,4),
        ...
        for _, fam in grp_df.drop_duplicates("영업가족").iterrows():
            ...
            E += [Paragraph("▶ 양식별 미처리 요약", section_left), _tbl(sum_d,[90,90,90,90],fn, align="LEFT"), Spacer(1,8),
                  ...
                  Paragraph(f"기준일: {DATA_REFERENCE_DATE}", center_date_style)]
...
def ledger_excel(families_by_dept, period_text, df_src):
    ...
    ws0.merge_cells("A1:F1"); ws0["A1"]=f"관리대장 목차  ·  기준일: {DATA_REFERENCE_DATE}"
    ...
    for dept_name,grp_df in families_by_dept.items():
        ...
        ws_c.cell(r,1,f"기준일: {DATA_REFERENCE_DATE}").font=bf; r+=2
        ...
        for _,fam in grp_df.drop_duplicates("영업가족").iterrows():
            ...
            ws_f.cell(r_f,1,f"기준일: {DATA_REFERENCE_DATE}").font=bf; r_f+=1
...
def dashboard_page():
    ...
    with col2:
        st.info(f"📅 기준: **{DATA_REFERENCE_DATE}**")  # ✅ 여기서 사용!
    with col3:
        if st.button("🔄 새로고침"):
            st.cache_data.clear()
            st.rerun()
    ...
    if not st.session_state.get("logged_in"):
        ...
        with st.sidebar:
            ...
            if st.button("🚪 로그아웃", use_container_width=True):
                ...
                st.rerun()
            st.divider()
            st.caption(f"v5.1 | 기준일:{DATA_REFERENCE_DATE} | © 2026")
        dashboard_page()

...
def _tbl(data, cw, fn, header_rows=1, sub_rows=None, align="CENTER"):
    ...
    S = getSampleStyleSheet()
    cell_style = ParagraphStyle("tbl_cell", parent=S["Normal"], fontName=fn, fontSize=7.5, leading=9.5, alignment=align_value, wordWrap="NONE")
...
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="그룹형", dash_top_n=15, map_type="🔲 트리맵"):
    ...
    col_name = map_level if map_level in map_agg.columns else map_agg.columns[0]
    ...
    if not map_agg.empty:
        ...
        hdr = [[col_name, "미스캔", "미처리율", "대상건"]]
        rows = [[r[col_name], f"{int(r['미스캔'])}", f"{r['미처리율']:.1f}", f"{int(r['대상건'])}"] for _, r in map_agg.iterrows()]
        E.append(_tbl(hdr + rows, [100, 55, 55, 55], fn)); E.append(Spacer(1,8))
        try:
            if map_type == "🥧 원그래프":
                fig_map = px.pie(map_agg, values="미스캔", names="조직", title=f"{map_level}별 미스캔 건수 비중", hole=0.4, color_discrete_sequence=px.colors.qualitative.Set3)
                fig_map.update_traces(textposition='inside', textinfo='percent+label')
            else:
                fig_map = px.treemap(map_agg, path=["조직"], values="미스캔", color="미처리율", title=f"{map_level}별 미스캔 분포", color_continuous_scale="RdYlGn_r")
...
def dashboard_page():
    ...
    with col1: st.success(f"✅ 총 **{len(df):.0f}건**의 데이터 로드 완료")
    ...
    m1.metric("📄 총 계약건수", f"{stats_all['대상건']:.0f}건")
    m2.metric("🧮 대상스캔건", f"{stats_all['대상스캔건']:.0f}건")
    m3.metric("📈 스캔율", f"{stats_all['스캔율']:.1f}")
    m4.metric("📉 미처리율", f"{stats_all['미처리율']:.1f}")
    st.caption(f"총 미처리건수: {stats_all['총미스캔']:.0f}건 | 개인정보동의서 집계 제외")
    st.divider()
    ...
    with tab_dash:
        ...
        if agg.empty: 
            ...
            st.dataframe(agg[["순위","조직","대상건","대상스캔건","전체스캔","총미스캔","스캔율","미처리율","FA_miss","비교_miss","완판_miss"]].style.format({
                "순위":"{:.0f}","대상건":"{:.0f}","대상스캔건":"{:.0f}","전체스캔":"{:.0f}","총미스캔":"{:.0f}","스캔율":"{:.1f}","미처리율":"{:.1f}",
                "FA_miss":"{:.0f}","비교_miss":"{:.0f}","완판_miss":"{:.0f}"
    ...
    with tab_map:
        ...
        if map_agg.empty: st.info("미처리 건수가 있는 데이터가 없습니다.")
            ...
            st.dataframe(map_agg.style.format({"미스캔":"{:.0f}","대상건":"{:.0f}","미처리율":"{:.1f}"}), use_container_width=True, hide_index=True)
    ...
    with tab_report:
        ...
        if report_df.empty: st.info("데이터가 없습니다.")
            ...
            st.dataframe(disp_df.style.apply(style_row, axis=1).format({
                "대상건":"{:.0f}","대상스캔건":"{:.0f}","전체스캔":"{:.0f}","총미스캔":"{:.0f}","스캔율":"{:.1f}","미처리율":"{:.1f}"
            ...
            if not pivot_df.empty:
                ...
                st.dataframe(pivot_display.style.format({col: "{:.0f}" for col in pivot_display.columns if col not in ["구분","부문","총괄","부서"] and not col.endswith("_미처리율")}).format({col: "{:.1f}" for col in pivot_display.columns if col.endswith("_미처리율")}), use_container_width=True, hide_index=True, height=420)
    ...
    with tab_ledger:
        ...
        if not targets: st.success("✅ 미스캔 발생 대상이 없습니다.")
            ...
            st.dataframe(prev_df.style.format({"FA":"{:.0f}","비교":"{:.0f}","완판":"{:.0f}","총미스캔":"{:.0f}"}), use_container_width=True, hide_index=True)
...
def main():
    if not st.session_state.get("logged_in"): login_page()
        ...
        with st.sidebar:
            ...
            st.divider()
            st.caption("v5.2 | KeyError해결·100%필터·검색메시지·형식통일 | © 2026")
        dashboard_page()

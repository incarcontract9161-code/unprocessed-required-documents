...
# 대상스캔건 = FA고지 + 비교설명 + 완판 대상 (해당사항없음 제외)
# FA고지와 비교설명는 모두 대상, 완판은 대상인 것만
대상스캔건 = FA고지_대상 + 비교설명_대상 + 완전판매_대상

# 스캔건수 = FA고지_스캔 + 비교설명_스캔 + 완전판매_스캔
전체스캔 = FA고지_스캔 + 비교설명_스캔 + 완전판매_스캔
...
rows.append({
 ...
 "총미스캔 ": stats[ "FA고지_미스캔 "] + stats[ "비교설명_미스캔 "] + stats[ "완전판매_미스캔 "],
})
...
rows.append({ "구분 ": "부문계 ",  "부문 ":bm,  "총괄 ": " ",  "부서 ": " ",  "영업가족 ": " ",
 "증번수 ":stats[ "증번수 "],  "대상건 ":stats[ "대상건 "],  "대상스캔건 ":stats[ "대상스캔건 "],
 "전체스캔 ":stats[ "전체스캔 "],  "스캔율 ":stats[ "스캔율 "]})
...
return report.sort_values([ "스캔율 ",  "증번수 "], ascending=[True, False]).reset_index(drop=True)
...
ws.merge_cells( "A1:I1 "); ws[ "A1 "] = f "서류 미처리 현황 계층별 집계  ·  기간: {period_str}  ·  발급: {today} "
ws[ "A1 "].font = Font(name=tfn,size=12,bold=True); ws[ "A1 "].alignment = Alignment(horizontal= "center "); ws.row_dimensions[1].height = 22
# 수정: 대상스캔건, 스캔율 추가, 개인정보 제외
headers = [ "구분 ", "부문 ", "총괄 ", "부서 ", "영업가족 ", "증번수 ", "대상건 ", "대상스캔건 ", "전체스캔 ", "스캔율 "]
...
for _, row in report.iterrows(): 
    ...
    vals = [gbn, row[ "부문 "], row[ "총괄 "], row[ "부서 "], row[ "영업가족 "], row[ "증번수 "], row[ "대상건 "], row[ "대상스캔건 "], row[ "전체스캔 "], rate_str]
...
today, period_str = datetime.now().strftime( "%Y년 %m월 %d일 "),  ",  ".join(months) if months else  "전체 "
E = [Paragraph( "서류 미처리 현황 계층별 집계 ", st [ "title "]), Paragraph(f "기간: {period_str}  |  발급일자: {today} ", st_[ "date "]), HRFlowable(width= "100% ",thickness=1,color=colors.HexColor(HDR_CLR)), Spacer(1,6)]
report = build_hierarchy_report(df, months)
if not report.empty:
E.append(Paragraph( "▶ 부문 / 총괄 / 부서 / 영업가족 계층 집계 ", st_[ "section "]))
# 수정: 대상스캔건, 스캔율 추가
hdr=[[ "구분 ", "부문 ", "총괄 ", "부서 ", "영업가족 ", "증번수 ", "대상건 ", "대상스캔건 ", "전체스캔 ", "스캔율 "]]
drows, sub_idx = [], []
...
monthly = build_monthly_hierarchy(df, months)
if not monthly.empty:
E.append(PageBreak()); E.append(Paragraph( "▶ 월별 계층별 미처리 집계 ", st [ "section "]))
mrows=[[
r[ "월 "],
r[ "구분 "],
r[ "부문 "],
...
today, period_str = datetime.now().strftime( "%Y년 %m월 %d일 "),  ",  ".join(months) if months else  "전체 "
E = [
Paragraph( "전체 페이지 요약 리포트 ", st [ "title "]),
...
# 수정: 개인정보 제외, 대상스캔건, 스캔율 중심
E.append(_tbl([
    [ "총 계약건수 ", f "{stats_all['증번수']:,} "],
    [ "대상건 ", f "{stats_all['대상건']:,} "],
    [ "대상스캔건 ", f "{stats_all['대상스캔건']:,} "],
    [ "전체스캔 ", f "{stats_all['전체스캔']:,} "],
    [ "스캔율 ", f "{stats_all['스캔율']:.1f}% "],
], [90, 120], fn, header_rows=0, align= "LEFT "))
E.append(Spacer(1, 8))

agg = build_group_scan_stats(df_sel, agg_group).sort_values( "스캔율 ", ascending=True).head(dash_top_n)
if not agg.empty:
    E.append(Paragraph(f "▶ 현황 대시보드 집계 ({agg_group}) ", st_[ "section "]))
    # 수정: 대상스캔건, 스캔율 추가
    hdr = [[ "조직 ", "대상건 ", "대상스캔건 ", "전체스캔 ", "스캔율 ", "FA고지_미스캔 ", "비교설명_미스캔 ", "완전판매_미스캔 "]]
    rows = [[r[ "조직 "], f "{int(r['대상건']):,} ", f "{int(r['대상스캔건']):,} ", f "{int(r['전체스캔']):,} ", f "{r['스캔율']:.1f}% ", f "{int(r['FA고지_미스캔']):,} ", f "{int(r['비교설명_미스캔']):,} ", f "{int(r['완전판매_미스캔']):,} "] for _, r in agg.iterrows()]
    E.append(_tbl(hdr + rows, [80, 40, 52, 40, 36, 42, 48, 48], fn))
    E.append(Spacer(1, 8))
...
if not report.empty:
    ...
    # 수정: 대상스캔건, 스캔율 추가
    hdr = [[ "구분 ", "부문 ", "총괄 ", "부서 ", "영업가족 ", "증번수 ", "대상건 ", "대상스캔건 ", "전체스캔 ", "스캔율 "]]
    drows = [[r[ "구분 "], r[ "부문 "], r[ "총괄 "], r[ "부서 "], r[ "영업가족 "], f "{int(r['증번수']):,} ", f "{int(r['대상건']):,} ", f "{int(r['대상스캔건']):,} ", f "{int(r['전체스캔']):,} ", f "{r['스캔율']:.1f}% "] for _, r in report.iterrows()]
    E.append(_tbl(hdr + drows, [30, 40, 40, 40, 55, 22, 24, 28, 24, 22], fn))
...
center_date_style = ParagraphStyle( "CenterDate ", parent=st [ "date "], alignment=1)
...
E.append(Paragraph( "(해당 데이터 없음) ", st [ "body "]))
E.append(Spacer(1,6))
...
Paragraph( "【필수 서류 상세 안내】 ", st [ "section "]),
_tbl(REQUIRED_DOCS_TABLE, [12, 60, 90, 198], fn, header_rows=1, align= "LEFT "), Spacer(1,8),
Paragraph(GUIDANCE_TEXT, notice_left), Spacer(1,8),
...
# KPI 메트릭 - 수정: 개인정보 제외, 대상스캔건, 스캔율 추가
stats_all = calculate_scan_stats(df_sel)
...
m2.metric( " 대상건 ", f "{stats_all['대상건']:,}건 ")
m3.metric( "📊 대상스캔건 ", f "{stats_all['대상스캔건']:,}건 ")
m4.metric( "📈 스캔율 ", f "{stats_all['스캔율']:.1f}% ")
st.caption(f "총 미처리건수: {stats_all['FA고지_미스캔']+stats_all['비교설명_미스캔']+stats_all['완전판매_미스캔']:,}건 | FA / 비교 / 완판: {fa_t:,} / {bi_t:,} / {cs_t:,} ")
st.divider()
...
with tab_dash:
    ...
    # 수정: 스캔율 기준 정렬 (낮은 순)
    agg = agg.sort_values( "스캔율 ", ascending=True).reset_index(drop=True); agg.insert(0, "순위 ",range(1,len(agg)+1))
    if agg.empty: st.info( "조건에 맞는 데이터가 없습니다. ")
    else:
        # 수정: 대상스캔건, 스캔율 추가, 개인정보 제외
        st.dataframe(
            agg[[ "순위 ", "조직 ", "대상건 ", "대상스캔건 ", "전체스캔 ", "스캔율 ", "FA고지_미스캔 ", "비교설명_미스캔 ", "완전판매_미스캔 "]]
            .style.format({
                 "순위 ":  "{:,} ",
                 "대상건 ":  "{:,} ",
                 "대상스캔건 ":  "{:,} ",
        ...
        c1, c2 = st.columns(2)
        with c1:
            doc_types = st.multiselect( "표시 서류 ", [ "FA고지 ", "비교설명 ", "완전판매 "], default=[ "FA고지 "])
            if doc_types:
                if len(doc_types)==1:
                    ...
                    cm2 = { "FA고지 ": "FA고지_미스캔 ", "비교설명 ": "비교설명_미스캔 ", "완전판매 ": "완전판매_미스캔 "}
                    p = top[[ "조직 "]+[cm2[d] for d in doc_types]].copy(); p.columns=[ "조직 "]+doc_types; p=p.melt( "조직 ",var_name= "종류 ",value_name= "건수 ")
                    fig = px.bar(p, x= "조직 ", y= "건수 ", color= "종류 ", barmode= "group " if ct== "그룹형 " else  "stack ", color_discrete_map={ "FA고지 ": "#FF6B6B ", "비교설명 ": "#4ECDC4 ", "완전판매 ": "#45B7D1 "})
                    fig.update_layout(xaxis_tickangle=-45, height=420); st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig2 = go.Figure(); fig2.add_trace(go.Scatter(x=top[ "조직 "], y=top[ "스캔율 "], mode= "lines+markers ", line=dict(shape= "spline ", color= "#2E75B6 "), marker=dict(size=6)))
            fig2.update_layout(title=f "스캔율 TOP {top_n} ", xaxis_tickangle=-45, yaxis=dict(range=[0, 100]), height=420); st.plotly_chart(fig2, use_container_width=True)
...
with tab_map:
    ...
    # 수정: build_group_scan_stats 사용
    map_agg = build_group_scan_stats(df_sel, map_level)
    map_agg[ "미스캔 "] = map_agg[ "FA고지_미스캔 "] + map_agg[ "비교설명_미스캔 "] + map_agg[ "완전판매_미스캔 "]
    ...
    if map_agg.empty: st.info( "미처리 건수가 있는 데이터가 없습니다. ")
        ...
        if map_type ==  "🥧 원그래프 ":
            ...
            fig_tree = px.treemap(map_agg, path=[ "조직 "], values= "미스캔 ", color= "스캔율 ", title=f "{map_level}별 미처리 분포 ", color_continuous_scale= "RdYlGn_r ")
            fig_tree.update_layout(height=500); st.plotly_chart(fig_tree, use_container_width=True)
        # 수정: 대상스캔건, 스캔율 추가
        st.markdown(f "#### 📊 {map_level}별 상세 데이터 ")
        st.dataframe(
            map_agg[[ "조직 ", "대상건 ", "대상스캔건 ", "전체스캔 ", "스캔율 ", "FA고지_미스캔 ", "비교설명_미스캔 ", "완전판매_미스캔 "]].style.format({
                 "대상건 ":  "{:,} ",
                 "대상스캔건 ":  "{:,} ",
                 "전체스캔 ":  "{:,} ",
                 "스캔율 ":  "{:.1f}% ",
...
st.rerun()
st.divider()
st.caption("v5.1 | 개인정보집계제외·대상스캔건추가·영업가족까지 | © 2026")
dashboard_page()

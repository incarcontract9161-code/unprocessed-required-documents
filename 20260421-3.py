...
def calculate_scan_stats(df_group):
    cnt = len(df_group)
    # [수정] 개인정보 제외: FA고지, 비교설명, 완판만 대상/미스캔/집계 포함
    FA고지_대상 = cnt
    비교설명_대상 = cnt
    ...
    전체_대상 = FA고지_대상 + 비교설명_대상 + 완전판매_대상
    전체_스캔 = FA고지_스캔 + 비교설명_스캔 + 완전판매_스캔
    전체_미스캔 = 전체_대상 - 전체_스캔
    스캔율 = round((전체_스캔 / 전체_대상 * 100), 1) if 전체_대상 > 0 else 0.0
    
    return {
        "증번수": cnt,
        "개인정보_대상": 0, "개인정보_스캔": 0, "개인정보_미스캔": 0,  # [수정] 집계 제외
...
def build_group_scan_stats(df, group_col):
    ...
    for org, df_group in df.groupby(group_col):
        ...
        rows.append({
            "조직": org, "대상건": stats["증번수"], "스캔대상건수": stats["전체_대상"], "전체스캔": stats["전체_스캔"],
            "총_미스캔": stats["전체_미스캔"], "스캔율": stats["스캔율"], "미처리율": round(100 - stats["스캔율"], 1),
            "FA고지_미스캔": stats["FA고지_미스캔"], "비교설명_미스캔": stats["비교설명_미스캔"], "완전판매_미스캔": stats["완전판매_미스캔"],
        })
...
def build_hierarchy_report(df, months=None):
    ...
    def recursive_agg(df_grp, level_idx, path_vals):
        if level_idx >= len(levels):  # 영업가족 상세 (소계 없음)
            for val, grp in df_grp.groupby("영업가족", dropna=False):
                stats = calculate_scan_stats(grp)
                row = {col: path_vals[i] if i < len(path_vals) else "" for i, col in enumerate(levels)}
                row["영업가족"] = val
                row.update({"구분": "영업가족", "증번수": stats["증번수"], "전체대상": stats["전체_대상"], 
                            "전체스캔": stats["전체_스캔"], "전체미스캔": stats["전체_미스캔"], "스캔율": stats["스캔율"]})
                rows.append(row)
        else:
            ...
            for val, grp in df_grp.groupby(col, dropna=False):
                ...
                row.update({"증번수": stats["증번수"], "전체대상": stats["전체_대상"], 
                            "전체스캔": stats["전체_스캔"], "전체미스캔": stats["전체_미스캔"], "스캔율": stats["스캔율"]})
                rows.append(row)
    ...
    total_row["구분"] = "총계"
    total_row.update({"증번수": total_stats["증번수"], "전체대상": total_stats["전체_대상"], 
                      "전체스캔": total_stats["전체_스캔"], "전체미스캔": total_stats["전체_미스캔"], "스캔율": total_stats["스캔율"]})
...
def build_monthly_hierarchy(df, months=None):
    ...
    has_sosok = "소속" in src.columns
    idx_cols = ["부문", "총괄", "부서"] + (["소속"] if has_sosok else [])
    
    agg = src.groupby(["월_피리어드"] + idx_cols).agg(
        FA=("FA_miss", "sum"), 비교=("비교_miss", "sum"), 완판=("완판_miss", "sum"),
        대상건=("증권번호", "count")
    ).reset_index()
    ...
    # [수정] 월별 가로 피벗 처리
    pivot_cols = ["총미스캔"]
    pivoted = agg.pivot_table(index=idx_cols + ["구분"], columns="월_피리어드", values=pivot_cols, fill_value=0)
...
def _tbl(data, cw, fn, header_rows=1, sub_rows=None, align="CENTER"):
    ...
    cw_scaled = [w * 1.7 for w in cw]  # [수정] 표 width 최대 확장
...
def report_excel(df, months):
    ...
    headers = ["구분", "부문", "총괄", "부서"] + (["소속"] if has_sosok else []) + ["영업가족", "증번수", "전체대상", "전체스캔", "전체미스캔", "스캔율"]
    cws = [14,20,20,20] + ([18] if has_sosok else []) + [24,12,12,12,14,16]
    ...
    for _, row in report.iterrows(): 
        ...
        vals = [gbn, row["부문"], row["총괄"], row["부서"]] + ([row.get("소속","")] if has_sosok else []) + [row["영업가족"], row["증번수"], row["전체대상"], row["전체스캔"], row["전체미스캔"], rate_str]
...
def report_pdf(df, months):
    ...
    if not report.empty:
        ...
        hdr = [["구분", "부문", "총괄", "부서"] + (["소속"] if has_sosok else []) + ["영업가족", "증번수", "전체대상", "전체스캔", "전체미스캔", "스캔율"]]
        drows, sub_idx = [], []
        for i,(_,r) in enumerate(report.iterrows()):
            drows.append([r["구분"], r["부문"], r["총괄"], r["부서"]] + ([r.get("소속","")] if has_sosok else []) + [r["영업가족"],
                        f"{int(r['증번수']):,}", f"{int(r['전체대상']):,}", f"{int(r['전체스캔']):,}", f"{int(r['전체미스캔']):,}", f"{r['스캔율']:.1f}%"])
            if r["구분"] in ("부문계", "총괄계", "부서계", "소속계"): sub_idx.append(i+1)
        col_w = [24,32,32,32] + ([22] if has_sosok else []) + [45,22,22,22,26,22]
...
# ==========================================
# 8-1. [추가] Plotly 차트를 ReportLab Image로 변환하는 헬퍼
# ==========================================
def plotly_to_image(fig, width_mm=160, height_mm=90):
    try:
        ...
        return Paragraph("<i>차트 렌더링 환경 미준비 (kaleido 필요)</i>", pdf_styles(register_korean_font())["notice"])
...
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="그룹형", dash_top_n=15, map_type="🔲 트리맵"):
    ...
    # [수정] 대시보드 차트 이미지 생성 및 삽입
    agg = build_group_scan_stats(df_sel, agg_group).sort_values("총_미스캔", ascending=False).head(dash_top_n)
    if not agg.empty:
        fig1 = go.Figure()
        fig1.add_trace(go.Bar(x=agg["조직"], y=agg["총_미스캔"], marker_color="#FF6B6B", text=agg["총_미스캔"], textposition="outside"))
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=agg["조직"], y=agg["총_미스캔"], mode="lines+markers", line=dict(color="#2E75B6", width=3), marker=dict(size=6)))
    ...
    if not report.empty:
        ...
        drows = [[r["구분"], r["부문"], r["총괄"], r["부서"]] + ([r.get("소속","")] if has_sosok else []) + [r["영업가족"],
                    f"{int(r['증번수']):,}", f"{int(r['전체대상']):,}", f"{int(r['전체스캔']):,}", f"{int(r['전체미스캔']):,}", f"{r['스캔율']:.1f}%"] for _, r in report.iterrows()]
        col_w = [24,32,32,32] + ([22] if has_sosok else []) + [45,22,22,22,26,22]
        ...
        if map_agg.empty: st.info("미처리 건수가 있는 데이터가 없습니다.")
            ...
            st.dataframe(
                map_agg[["조직", "대상건", "스캔대상건수", "전체스캔", "미스캔", "스캔율", "미처리율"]].style.format({
                    "대상건": "{:,}", "스캔대상건수": "{:,}", "전체스캔": "{:,}", "미스캔": "{:,}", "스캔율": "{:.1f}%", "미처리율": "{:.1f}%"
                }), use_container_width=True, hide_index=True)
    ...
    with tab_report:
        ...
        if report_df.empty: st.info("데이터가 없습니다.")
            ...
            def style_row(row):
                ...
                return [""]*len(row)
            disp_cols = ["구분", "부문", "총괄", "부서"] + (["소속"] if "소속" in report_df.columns else []) + ["영업가족", "증번수", "전체대상", "전체스캔", "전체미스캔", "스캔율"]
            ...
            st.caption("스캔 처리: 스캔, M스캔, 보험사스캔 / 완판대상: 스캔, M스캔, 미스캔만 대상 / 개인정보는 집계 제외")
            st.dataframe(
                disp_df.style.apply(style_row, axis=1).format({
                    "증번수": "{:,}", "전체대상": "{:,}", "전체스캔": "{:,}", "전체미스캔": "{:,}", "스캔율": "{:.1f}%"

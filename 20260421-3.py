...
REQUIRED_DOCS_TABLE = [
    ...
    ["2", "비교설명확인서",
     "보험업감독규정\n별표 5-6",
     "유사 상품 3 개 이상 비교·설명 이행\n사실 고객 확인 서명"],
    ["3", "고지의무확인서",
     "금융소비자보호법 26 조와\n동법시행령 24 조",
     "판매자 중요사항 고지의무 이행 확인,\n권한·책임·보상 관련 핵심 사항 고지,\n소비자 오인 예방"],
    ["4", "완전판매확인서\n(대상: 종신, CI, CEO 정기, 고액)",
     "금융소비자보호법 제 17·19 조 설명 적합성 적정성 관련 조항\n영업지원기준안",
...
def load_data():
    if not os.path.exists(EXCEL_FILE):
        st.error(f"⚠️ '{EXCEL_FILE}' 파일이 없습니다. GitHub 에 엑셀 파일을 업로드해주세요.")
        return pd.DataFrame()
    ...
    try:
        ...
        # [추가] 스캔 상태 판별 함수
        def is_scanned(val):
            return str(val).strip() in ["스캔", "M 스캔", "보험사스캔"] if pd.notna(val) and str(val).strip() != "" else False
...
# ==========================================
# 3. 집계 헬퍼 [수정: 대상건/스캔대상건/스캔율 분리]
# ==========================================
# [추가] 증번 기준 스캔율 계산 함수
def calculate_scan_stats(df_group):
    """
    - 대상건: 증권개수 (cnt)
    - 대상스캔건: FA+ 비교 + 완판 (해당사항없음 제외) 대상 합계
    - 전체스캔: 3 개 서류 중 '스캔, M 스캔, 보험사스캔' 처리된 건수 합계
    - 스캔율 = 전체스캔 / 대상스캔건 * 100
    - 미처리율 = 100 - 스캔율
    - 개인정보동의서는 집계 제외
    """
    ...
    # 1) 대상스캔건 계산 (개인정보 제외, 완판은 해당사항없음 제외)
    FA_대상 = cnt
    비교_대상 = cnt
    완판_대상 = int(df_group["완판_대상"].sum())  # 해당사항없음 제외
    대상스캔건 = FA_대상 + 비교_대상 + 완판_대상
    
    # 2) 전체스캔 계산 (스캔, M 스캔, 보험사스캔 만 카운트)
...
# ==========================================
# 4. 전체 계층 리포트 [수정: 영업가족까지만, 100% 완료 필터]
# ==========================================
@st.cache_data(ttl=300)
        ...
        for bm, db in dm.groupby("부문"):
            ...
            for tg, dt in db.groupby("총괄"):
                ...
                rows.append({"월":mon, "구분":"총괄계", "부문":bm, "총괄":tg, "부서":"",
                             ...
                             "대상건":cnt2, "미처리율":round(tot2/cnt2*100,1) if cnt2 else 0.0})
                for ds, dd in dt.groupby("부서"):
                    ...
                    rows.append({"월":mon, "구분":"부서계", "부문":bm, "총괄":tg, "부서":ds,
                                 ...
                                 "대상건":cnt3, "미처리율":round(tot3/cnt3*100,1) if cnt3 else 0.0})
    return pd.DataFrame(rows)
...
# ==========================================
# 7. 전체 계층 리포트 Excel [수정: 대상스캔건/스캔율 컬럼 추가]
# ==========================================
def report_excel(df, months):
    ...
    today, period_str = datetime.now().strftime("%Y 년 %m 월 %d 일"), ", ".join(months) if months else "전체"
    
    ws.merge_cells("A1:M1"); ws["A1"] = f"서류 미처리 현황 계층별 집계  ·  기간: {period_str}  ·  발급: {today}"
    ws["A1"].font = Font(name=tfn,size=12,bold=True); ws["A1"].alignment = Alignment(horizontal="center"); ws.row_dimensions[1].height = 22
    # [수정] 대상건, 대상스캔건, 전체스캔, 스캔율, 미처리율 컬럼 추가
...
# ==========================================
# 8. 전체 계층 리포트 PDF [수정: 대상스캔건/스캔율 반영]
# ==========================================
def report_pdf(df, months):
    ...
    if not report.empty:
        ...
        # [수정] 대상건, 대상스캔건, 전체스캔, 스캔율, 미처리율 컬럼 반영
        ...
        E.append(_tbl(hdr+drows,[40,52,52,52,72,29,32,29,32,26,26],fn,sub_rows=sub_idx)); E.append(Spacer(1,8))
    monthly = build_monthly_hierarchy(df, months)
    if not monthly.empty:
    ...
    if not pivot.empty:
        ...
        col_count = len(pivot.columns); fixed = [24, 24, 30, 40]
        month_cols = max(1, col_count - 4); remaining = max(12, int((542 - sum(fixed)) / month_cols))
        E.append(_tbl(hdr + values, fixed + [remaining] * month_cols, fn))
    doc.build(E); buf.seek(0); return buf

# ==========================================
# 9. 전체 페이지 PDF [수정: 스캔율/미처리율 차트 반영]
# ==========================================
def report_fullpage_pdf(df, months, agg_group, map_level, dash_doc_types=None, dash_chart_mode="그룹형", dash_top_n=15, map_type="🔲 트리맵"):
    ...
    if not agg.empty:
        ...
        E.append(_tbl(hdr + rows, [90, 40, 52, 40, 40, 36, 36], fn)); E.append(Spacer(1,8))
        try:
            # [수정] 스캔율 기반 차트 (옵션)
            fig_dash = go.Figure()
    ...
    if not pivot.empty:
        ...
        fixed = [30, 30, 35, 45]; month_cols = max(1, len(headers) - 4)
...
def ledger_pdf(families_by_dept, period_text, df_src):
    ...
    today = datetime.now().strftime("%Y 년 %m 월 %d 일"); E = []
    ...
    for dept_name, grp_df in families_by_dept.items():
        ...
        for _, fam in grp_df.drop_duplicates("영업가족").iterrows():
            ...
            E.append(Spacer(1,6))
            sum_d=[["FA 고지","비교설명","완전판매","총계"],[f"{int(fam['FA']):,}",f"{int(fam['비교']):,}",f"{int(fam['완판']):,}",f"{int(fam['총미스캔']):,}"]]
...
def ledger_excel(families_by_dept, period_text, df_src):
    ...
    today = datetime.now().strftime("%Y 년 %m 월 %d 일")
    ...
    for ci,h in enumerate(["부서","영업가족","FA 고지","비교설명","완전판매","총미스캔"],1):
    ...
    for dept_name,grp_df in families_by_dept.items():
        ...
        r += 1
        docs_data = [
            ["1", "개인정보동의서", "개인정보보호법 15 조 및\n대리점 표준 내부통제기준 27 조", "개인정보 처리 적법 근거, 보유계약 전산 관리 과정에 따른 개인정보 처리로 신계약시 필수 징구"],
            ["2", "비교설명확인서", "보험업감독규정\n별표 5-6", "유사 상품 3 개 이상 비교·설명 이행 사실 고객 확인 서명"],
            ["3", "고지의무확인서", "금융소비자보호법 26 조와\n동법시행령 24 조", "판매자 권한·책임·보상 관련 핵심 사항 고지, 소비자 소인 예방"],
            ["4", "완전판매확인서\n(대상: 종신, CI, CEO 경기, 고액)", "금융소비자보호법 제 17·19 조\n영업지원기준안", "약관, 청약서 부본 제공, 중요 상품 이해 및 자발적 가입 확인, 설명 의무 이행 증빙력 확보"]
        ]
        ...
        if not dept_src.empty:
            fam_mon=dept_src.groupby(["영업가족","월_피리어드"]).agg(FA=("FA_miss","sum"),비교=("비교_miss","sum"),완판=("완판_miss","sum")).reset_index()
            fam_mon["계"]=fam_mon[["FA","비교","완판"]].sum(axis=1); fam_mon=fam_mon[fam_mon["계"] > 0]
            hdrs, cws = ["영업가족","월","FA 고지","비교설명","완전판매","계"], [25,20,13,13,13,13]
        ...
        for _,fam in grp_df.drop_duplicates("영업가족").iterrows():
            ...
            sosok=fam_src.groupby(["소속","월_피리어드"]).agg(FA=("FA_miss","sum"),비교=("비교_miss","sum"),완판=("완판_miss","sum")).reset_index()
            sosok["계"]=sosok[["FA","비교","완판"]].sum(axis=1); sosok=sosok[sosok["계"] > 0]
            sh, sc = ["소속","월","FA 고지","비교설명","완전판매","계"], [25,20,13,13,13,13]
            ...
            for ci,h in enumerate(["FA 고지","비교설명","완전판매","총계"],1): c=ws_f.cell(r_f,ci,h); c.font=hf; c.fill=h_fill; c.border=bdr; c.alignment=Alignment(horizontal="center")
...
def dashboard_page():
    ...
    df = load_data()
    if df.empty:
        st.warning("📭 데이터가 없습니다. GitHub 에 'insurance_data.xlsx' 파일을 업로드해주세요.")
        st.info("""
        **데이터 업로드 방법:**
        1. GitHub 저장소에 `insurance_data.xlsx` 파일 업로드
        2. Git commit & push
        3. Streamlit Cloud 가 자동으로 재배포 (1-2 분 소요)
        4. 페이지 새로고침
        """)
        return
    ...
    if not sel_months: st.warning("⚠️ 최소 1 개 이상의 월을 선택해주세요."); return
    ...
    with tab_dash:
        ...
        if agg.empty: st.info("조건에 맞는 데이터가 없습니다.")
            ...
            # [수정] 대상건, 대상스캔건, 스캔율, 미처리율 컬럼 반영
            st.dataframe(agg[["순위","조직","대상건","대상스캔건","전체스캔","총미스캔","스캔율","미처리율"]].style.format({
                "순위":"{:,}","대상건":"{:,}","대상스캔건":"{:,}","전체스캔":"{:,}","총미스캔":"{:,}","스캔율":"{:.1f}%","미처리율":"{:.1f}%"
            }), use_container_width=True, hide_index=True)
            top_n = st.slider("차트 표시 개수", 5, 30, 30, key="dash_top_n"); top = agg.head(top_n)
            c1, c2 = st.columns(2)
            with c1:
                doc_types = st.multiselect("표시 서류", ["FA 고지","비교설명","완전판매","총 미스캔"], default=["총 미스캔"], key="dash_doc_types")
                if doc_types:
                    ...
                    if len(doc_types)==1 and doc_types[0]=="총 미스캔":
                        ...
                        cm = {"FA 고지":"FA_miss","비교설명":"비교_miss","완전판매":"완판_miss","총 미스캔":"총미스캔"}
                        p = top[["조직"]+[cm[d] for d in doc_types]].copy(); p.columns=["조직"]+doc_types; p=p.melt("조직",var_name="종류",value_name="건수")
                        fig = px.bar(p, x="조직", y="건수", color="종류", barmode="group", color_discrete_map={"FA 고지":"#FF6B6B","비교설명":"#4ECDC4","완전판매":"#45B7D1","총 미스캔":"#9B59B6"})
    ...
    with tab_map:
        ...
        if map_agg.empty: st.info("미처리 건수가 있는 데이터가 없습니다.")
            ...
            st.dataframe(map_agg.rename(columns={map_level:"조직"}).style.format({"미스캔":"{:,}","대상건":"{:,}","미처리율":"{:.1f}%"}), use_container_width=True, hide_index=True)
    ...
    with tab_report:
        ...
        if report_df.empty: st.info("데이터가 없습니다.")
            ...
            def style_row(row):
                ...
                return [""]*len(row)
            # [수정] 대상건, 대상스캔건, 전체스캔, 스캔율, 미처리율 컬럼만 표시
    ...
    with tab_ledger:
        ...
        if not targets: st.success("✅ 미스캔 발생 대상이 없습니다.")
            ...
            if not sel_depts: st.warning("⚠️ 출력할 부서를 1 개 이상 선택하세요.")
            else:
...
def main():
    if not st.session_state.get("logged_in"): login_page()
        ...
        with st.sidebar:
            ...
            st.divider()
            st.caption("v5.1 | 대상건/스캔대상건분리·스캔율수정·100% 완료필터 | © 2026")
        dashboard_page()

...
def dashboard_page():
    ...
    # ✅ 탭 6 추가: FA별 M스캔 사용 현황
    tab_dash, tab_map, tab_target, tab_guide, tab_manual, tab_fa = st.tabs([
        "📊 현황 대시보드", "🗺️ M스캔 활용 현황", "🎯 목표관리 & 공문출력", 
        "📱 가이드 & 프로세스", "📚 매뉴얼 다운로드", "👥 FA별 사용현황"
    ])

    # ==========================================
    # 탭 1: 현황 대시보드 (✅ 비교 기준 연동 수정)
    # ==========================================
    with tab_dash:
        ...
        # ✅ 수정: compare_type 변경 시 기준치 실시간 연동
    ...
    # ==========================================
    # ✅ 탭 6: FA별 M스캔 사용 현황 (새로운 탭)
    # ==========================================
    with tab_fa:
        st.subheader("👥 FA별 M스캔 사용 현황 집계")
        
        if "FA사번" in df_sel.columns:
            # FA사번별 집계
            fa_stats = df_sel.groupby("FA사번").agg(
                총대상건=("대상건", "sum"),
                총M스캔건=("M스캔건", "sum"),
                M스캔사용여부=("M스캔건", lambda x: 1 if x.sum() > 0 else 0)
            ).reset_index()
            
            fa_stats.columns = ["FA사번", "총대상건", "총M스캔건", "M스캔사용여부"]
            fa_stats["M스캔율"] = safe_rate(fa_stats["총M스캔건"], fa_stats["총대상건"])
            
            # 사용/미사용 FA 통계
            total_fa = len(fa_stats)
            using_fa = len(fa_stats[fa_stats["M스캔사용여부"] == 1])
            not_using_fa = len(fa_stats[fa_stats["M스캔사용여부"] == 0])
            using_rate = (using_fa / total_fa * 100) if total_fa > 0 else 0
            
            st.markdown(f"### 📊 FA별 M스캔 사용 현황")
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("👨‍💼 총 FA 수", f"{total_fa}명")
            with col2:
                st.metric("✅ M스캔 사용 FA", f"{using_fa}명")
            with col3:
                st.metric("❌ 미사용 FA", f"{not_using_fa}명")
            with col4:
                st.metric("📈 사용률", f"{using_rate:.1f}%")
            
            st.divider()
            
            # FA별 상세 현황 테이블
            st.markdown("### 📋 FA별 상세 현황")
            display_fa_df = fa_stats[["FA사번", "총대상건", "총M스캔건", "M스캔율", "M스캔사용여부"]].copy()
            display_fa_df["M스캔사용여부"] = display_fa_df["M스캔사용여부"].apply(lambda x: "✅ 사용중" if x == 1 else "❌ 미사용")
            
            st.dataframe(
                display_fa_df.style.format({
                    "총대상건": "{:,}건",
                    "총M스캔건": "{:,}건",
                    "M스캔율": "{:.1f}%"
                }),
            ...
            with pd.ExcelWriter(buf, engine='openpyxl') as writer:
                fa_stats.to_excel(writer, sheet_name='FA별현황', index=False)
            buf.seek(0)
            st.download_button(
                "📥 FA별 현황 Excel 다운로드",
                data=buf,
                file_name=f"FA별_M스캔_사용현황_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
        else:
            st.warning("⚠️ 데이터에 'FA사번' 컬럼이 없습니다. Excel 파일에 FA사번 컬럼을 추가해주세요.")
...
def main():
    if not st.session_state.get("logged_in"): login_page()
        ...
        with st.sidebar:
            ...
            st.divider()
            st.caption("v15.8 | 기준치연동수정 | 목표달성필요건수 | FA별집계탭추가")
        dashboard_page()
